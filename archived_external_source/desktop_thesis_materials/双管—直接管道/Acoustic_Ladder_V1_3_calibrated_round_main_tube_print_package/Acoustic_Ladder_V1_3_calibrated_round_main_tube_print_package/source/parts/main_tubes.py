...
def _add_end_socket(body: cq.Workplane, x_face: float, direction: int,
    ...
    # Add the retaining ear before cutting the socket.  The V1.2 order added
    # the ear afterwards and refilled a small sector of the calibrated socket,
    # causing the end plug to collide near full insertion.
    return body.union(ear).cut(socket)
...
def _add_split_male(body: cq.Workplane, x_face: float,
    ...
    male = cone((x_face, y_center, 0.0), (1, 0, 0), base_d, tip_d,
                p.JOINT_MALE_LENGTH)
    # Start the key at the rigid shoulder.  This removes the short floating
    # underside in the print orientation and matches the successful coupon.
    key = box_centered(
        p.JOINT_MALE_LENGTH,
        p.JOINT_KEY_WIDTH,
        p.JOINT_KEY_HEIGHT,
        (x_face + p.JOINT_MALE_LENGTH / 2.0,
         y_center,
         base_d / 2.0 + p.JOINT_KEY_HEIGHT / 2.0
         - p.JOINT_KEY_RADIAL_OVERLAP),
    )
    shoulder = cylinder(
...
def _add_split_socket(body: cq.Workplane, x_face: float,
    ...
    key_slot = box_centered(
        ...
        (x_face + p.JOINT_SOCKET_DEPTH / 2.0,
         y_center,
         p.JOINT_KEY_SLOT_RADIAL_CENTER),
    )
    ear = box_centered(
        p.JOINT_LOCK_EAR_SIZE_X,
        p.JOINT_LOCK_EAR_SPAN_Y,
    ...
    # Cut the cone and keyway through the retaining ear as well.  Otherwise
    # the ear refills the final 0.8 mm of the keyway and blocks full insertion.
    return body.union(ear).cut(socket).cut(key_slot)
...
def equal_area_round_lumen_diameter() -> float:
    """Diameter of a circle with the same area as the V1.1 teardrop lumen."""
    area = (
        0.5 * math.pi * p.MAIN_INNER_RADIUS ** 2
        + p.MAIN_INNER_RADIUS * p.MAIN_ROOF_HEIGHT
    )
    return math.sqrt(4.0 * area / math.pi)


def make_tube_segment(
    role: str,
    section: str,
    lumen_profile: str = "teardrop",
) -> cq.Workplane:
    role = role.upper()
    section = section.lower()
    if role not in {"TX", "RX"} or section not in {"front", "rear"}:
        raise ValueError("role must be TX/RX and section front/rear")
    if lumen_profile not in {"teardrop", "round"}:
        raise ValueError("lumen_profile must be teardrop or round")
    ...
    # Exact BREP lumen cut, extended slightly through every mating boundary.
    if lumen_profile == "round":
        lumen = cylinder(
            (x0 - 0.2, y_center, 0.0),
            (1, 0, 0),
            equal_area_round_lumen_diameter(),
            lumen_end - x0 + 0.4,
        )
    else:
...
def all_main_tubes_round():
    """Round-lumen variant; all external interfaces remain V1.1-compatible."""
    return {
        "ALV1_TX_front_0_200": make_tube_segment("TX", "front", "round"),
        "ALV1_TX_rear_200_400": make_tube_segment("TX", "rear", "round"),
        "ALV1_RX_front_0_200": make_tube_segment("RX", "front", "round"),
        "ALV1_RX_rear_200_400": make_tube_segment("RX", "rear", "round"),
    }
