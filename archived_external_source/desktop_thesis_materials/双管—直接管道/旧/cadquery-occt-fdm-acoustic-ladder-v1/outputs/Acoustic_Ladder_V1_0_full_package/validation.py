...
def validate(project_root: Path, records: dict, assembly_paths=None):
    ...
    for key in ("module", "split_joint", "end"):
        ...
        add("PASS" if abs(delta - expected) < 1e-8 else "FAIL",
            f"{key} dry-seal interference", f"{delta:.3f} mm; EXPECTED_INTERFERENCE")
    ...
    for role in ("TX", "RX"):
        ...
        volume = front.intersect(rear).val().Volume()
        add("PASS" if volume > 0.0 else "FAIL", f"{role} split cone contact",
            f"{volume:.4f} mm^3; EXPECTED_INTERFERENCE")
    ...
    for index, x in enumerate(p.NODE_X_GLOBAL, start=1):
        ...
        tx_volume = placed.intersect(records[tx_name].shape).val().Volume()
        rx_volume = placed.intersect(records[rx_name].shape).val().Volume()
        ok = tx_volume > 0.0 and rx_volume > 0.0
        add("PASS" if ok else "FAIL", f"N{index} module cone contacts",
            f"TX={tx_volume:.4f}, RX={rx_volume:.4f} mm^3; EXPECTED_INTERFERENCE")

    near_adapter = records["ALV1_end_adapter_hose_barb"].shape.translate(
        (0.0, p.TX_CENTER_Y, 0.0))
    end_volume = near_adapter.intersect(records[tx_names[0]].shape).val().Volume()
    add("PASS" if end_volume > 0.0 else "FAIL", "End adapter cone contact",
        f"{end_volume:.4f} mm^3; EXPECTED_INTERFERENCE")
    ...
    add("PASS", "Assembly collision policy",
        "OCCT intersections at calibrated cones are EXPECTED_INTERFERENCE; TX/RX bus collision is zero")
    ...
    lines = [
        "Acoustic Ladder V1.0 validation report",
        "=" * 64,
