...
from geometry_utils import box_centered, compound, cone, cylinder, ground_z
from parts.supports import make_rx_slider, make_support_base, make_wedge
...
def make_slider_coupon() -> cq.Workplane:
    # Use the actual standard guide/stop geometry, not a symbolic rail.  Every
    # detachable piece is grounded separately so all five bodies start on Z=0.
    base = ground_z(make_support_base(False)).translate((-18.0, 0.0, 0.0))
    slider = ground_z(make_rx_slider(False)).translate((0.0, 15.0, 0.0))
    wedges = [
        ground_z(make_wedge(level)).translate((offset, -22.0, 0.0))
