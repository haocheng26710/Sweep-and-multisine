...
P: Dict[str, Any] = {
    'version': '2.5.0',
    'package_type': 'Patch/add-on for Acoustic Morphology Encoder V2.0.0',
    'date': '2026-07-28',
    'units': 'mm',
...
LID_MOD=clean(BODY_MOD.buffer(0.8,join_style='mitre'))
X0=-28.6; X1=28.6
...
for idx,spec in enumerate(HR_SPECS,1):
    ...
    lid=layered_mesh([(0,0.8,LID_MOD),(0.8,1.2,clean(LID_MOD.difference(dots)))],f'V25_{spec["id"]}_lid')
...
def make_plate(ids,name):
    ...
    for i,hid in enumerate(ids):
        ...
        trans.extend([(col*74,row*52,0),(col*74+38,row*52,0)])
    p=combine_meshes(meshes,trans)
    p.apply_translation(-p.bounds.mean(axis=0))
