...
VERSION = "V8R3T / chamber_bridge_fix_trimmed"
OUT = Path(__file__).resolve().parent
PACKAGE_ZIP = OUT / "v8_chamber_bridge_fix_trimmed_package.zip"
...
MINIMUM_EDGE_WALL_REQUIRED_MM = 5.0
MINIMUM_EDGE_WALL_PREFERRED_MM = 8.0
...
def functional_y_bounds():
    ...
    for points in LANES.values():
        extents.append(polyline_y_extent(points, MAIN_DUCT_MODELED_DIAMETER_MM))
    for bridge in BRIDGES.values():
        extents.append(polyline_y_extent(bridge["points"], BRIDGE_MODELED_DIAMETER_MM))
    extents.append((
        CHAMBER_CENTER_MM[1] - CHAMBER_HALF_HEIGHT_MM,
        CHAMBER_CENTER_MM[1] + CHAMBER_HALF_HEIGHT_MM,
    ))
    extents.append(polyline_y_extent([CHAMBER_CENTER_MM, MIC_PORT_END_MM], MIC_PORT_MODELED_DIAMETER_MM))
    return min(v[0] for v in extents), max(v[1] for v in extents)


def trim_bounds():
    functional_min, functional_max = functional_y_bounds()
    new_min = max(ORIGINAL_BODY_MIN_Y_MM, functional_min - TRIM_MARGIN_Y_MM)
    new_max = min(ORIGINAL_BODY_MAX_Y_MM, functional_max + TRIM_MARGIN_Y_MM)
...
def trim_report():
    ...
    min_edge_wall = min(functional_min - new_min, new_max - functional_max)
    return {
        ...
        "trim_margin_y_mm": TRIM_MARGIN_Y_MM,
        "minimum_channel_or_port_outer_wall_to_y_edge_mm": round(min_edge_wall, 3),
        "minimum_edge_wall_required_mm": MINIMUM_EDGE_WALL_REQUIRED_MM,
        "minimum_edge_wall_preferred_mm": MINIMUM_EDGE_WALL_PREFERRED_MM,
        "minimum_edge_wall_meets_required": min_edge_wall >= MINIMUM_EDGE_WALL_REQUIRED_MM,
        "minimum_edge_wall_meets_preferred": min_edge_wall >= MINIMUM_EDGE_WALL_PREFERRED_MM,
        ...
        "estimated_print_time_trend": "lower than V8R3 because unused top/bottom board area is removed; exact time depends on slicer settings",
...
def load_original_sim_params():
    path = OUT.parent / "v8_chamber_bridge_fix" / "sim_params_v8_chamber_bridge_fix.json"
    if not path.exists():
        return None
...
def acoustic_invariance_report():
    original = load_original_sim_params()
    ...
    if original is None:
        return {
            ...
            "note": "Original V8R3 sim_params not found next to this package; standalone geometry tables are unchanged in this generator.",
        }
    ...
    return {
        ...
        "comparison_source": str(OUT.parent / "v8_chamber_bridge_fix" / "sim_params_v8_chamber_bridge_fix.json"),
...
def validate(checks):
    ...
    report = {
        ...
        "minimum_edge_wall_meets_required_5mm": trim["minimum_edge_wall_meets_required"],
        "minimum_edge_wall_meets_preferred_8mm": trim["minimum_edge_wall_meets_preferred"],
        ...
        "no_channel_clipped_by_trim": trim["minimum_edge_wall_meets_required"],
        "no_acoustic_path_changed": acoustic_invariance["all_acoustic_geometry_unchanged"] is True,
        "thin_edge_region_warning": None if trim["minimum_edge_wall_meets_required"] else "Minimum outer wall to y edge is below 5 mm.",
...
def write_json_and_text(checks, validation):
    ...
    sim = {
        ...
        "layout": {
            "type": "V8R3T_external_board_trim_only",
            "main_duct_intent": "identical to V8R3; only unused top/bottom board margins are trimmed",
        ...
        "geometry_modification": {
            "type": "external_board_trim_only",
            "source_version": "V8R3 / chamber_bridge_fix",
            "acoustic_geometry_changed": False,
    ...
    (OUT / "sim_params_v8_chamber_bridge_fix_trimmed.json").write_text(json.dumps(sim, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V8R3T / chamber_bridge_fix_trimmed model parameters",
        "",
        "Design summary",
        "- V8R3T is the trimmed external-board version of V8R3.",
        "- Only unused top/bottom board margins are removed.",
        ...
        f"- trim_margin_y: {trim['trim_margin_y_mm']} mm",
        f"- Minimum channel/port outer wall to y edge: {trim['minimum_channel_or_port_outer_wall_to_y_edge_mm']} mm",
        f"- Minimum edge wall meets 5 mm required: {trim['minimum_edge_wall_meets_required']}",
        f"- Minimum edge wall meets 8 mm preferred: {trim['minimum_edge_wall_meets_preferred']}",
        f"- Estimated body volume savings: {trim['estimated_body_volume_savings_percent']} percent",
        f"- Estimated material savings: {trim['estimated_material_savings_percent']} percent",
    ...
    lines += [
        ...
        f"- minimum port outer wall to y edge: {validation['minimum_port_outer_wall_to_y_edge_mm']} mm",
    ...
    (OUT / "model_params_v8_chamber_bridge_fix_trimmed.txt").write_text("\n".join(lines) + "\n", encoding="utf-8")

    readme = f"""V8R3T / chamber_bridge_fix_trimmed acoustic network engineering package
=================================================================

Design purpose
--------------
This is the trimmed version of V8R3. It only removes unused top/bottom board
material. It does not change A/B/C/D main ducts, bridges, bridge taps, mixing
chamber, chamber inlets, input ports, mic port, centerline lengths, or any
acoustic path parameter.

Trim summary
------------
- Original y span: {trim['original_y_span_mm']} mm.
- Trimmed y span: {trim['trimmed_y_span_mm']} mm.
- New y bounds: {trim['new_y_min_mm']} to {trim['new_y_max_mm']} mm.
- Cut from bottom: {trim['cut_from_bottom_mm']} mm.
- Cut from top: {trim['cut_from_top_mm']} mm.
- Minimum channel/port outer wall to y edge: {trim['minimum_channel_or_port_outer_wall_to_y_edge_mm']} mm.
- Estimated material saving trend: {trim['estimated_material_savings_percent']} percent less board volume.

Experimental equivalence
------------------------
V8R3T is acoustically equivalent to untrimmed V8R3 for the intended experiment.
The sim parameters record geometry_modification.type = external_board_trim_only
and acoustic_geometry_changed = false.

STL files
---------
- V8R3T_S0_no_bridges.stl: baseline with no bridge.
- V8R3T_S1_AB_strong_only.stl: AB strong straight bridge only.
- V8R3T_S2_AB_sbridge_only.stl: AB S bridge only, same duct positions as S1.
- V8R3T_S3_BC_sbridge_only.stl: BC S bridge only.
- V8R3T_S4_CD_sbridge_only.stl: CD S bridge only.
- V8R3T_S5_AB_strong_BC_sbridge.stl: AB strong plus BC S bridge.
- V8R3T_S6_AB_strong_BC_CD_sbridges.stl: AB strong plus BC/CD S bridges.
- V8R3T_S7_AB_BC_CD_all_sbridges.stl: AB/BC/CD all S bridges.

Other files
-----------
- generate_v8_chamber_bridge_fix_trimmed.py: parametric generator.
- model_params_v8_chamber_bridge_fix_trimmed.txt: human-readable dimensions and checks.
- sim_params_v8_chamber_bridge_fix_trimmed.json: structured data for later 1D graph simulation.
- validation_report_v8_chamber_bridge_fix_trimmed.txt: generated validation report.
...
- Confirm all V8R3T states have the same external outline.
- Confirm no slicer support is generated inside the teardrop channels.

Important warning
-----------------
V8R3T is not a new acoustic topology. It is an external board trim of V8R3.
"""
    (OUT / "README_v8_chamber_bridge_fix_trimmed.txt").write_text(readme, encoding="utf-8")

    vlines = ["V8R3T / chamber_bridge_fix_trimmed validation report", ""]
    for key, value in validation.items():
        vlines.append(f"{key}: {value}")
    (OUT / "validation_report_v8_chamber_bridge_fix_trimmed.txt").write_text("\n".join(vlines) + "\n", encoding="utf-8")
