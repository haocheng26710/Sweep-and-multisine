...
VERSION = "V9.2 / half_skeletonized"
OUT = Path(__file__).resolve().parent
PACKAGE_ZIP = OUT / "V9_2_half_skeletonized_package.zip"
...
RIB_WIDTH_MM = 6.0
RIB_HEIGHT_MM = 5.8
RIB_X_CENTERS_MM = (28.0, 70.0, 112.0, 136.0)
...
def feature_y_extents():
    ...
    bridge_extents = {name: polyline_y_extent(bridge["points"], BRIDGE_MODELED_DIAMETER_MM) for name, bridge in BRIDGES.items()}
    chamber_extent = (
...
def _bridge_air_top_z():
    return DUCT_CENTER_Z_MM + TEARDROP_ROOF_RATIO * (BRIDGE_MODELED_DIAMETER_MM / 2.0)
...
def build_v9_2_masks():
    ...
    bridge_shell_radius = BRIDGE_MODELED_DIAMETER_MM / 2.0 + BRIDGE_SHELL_WALL_MM
...
def validate(checks):
    ...
    report = {
        ...
        "bridge_cross_section_unchanged": True,
        "bridge_cross_section": teardrop_props(BRIDGE_MODELED_DIAMETER_MM),
    ...
    report["validation_failure_conditions_passed"] = (
        ...
        and acoustic_invariance["all_acoustic_geometry_unchanged"] is True
...
def write_json_and_text(checks, validation):
    ...
    lines += [
        ...
        f"- Bridge cross-section unchanged: {validation['bridge_cross_section_unchanged']}",
