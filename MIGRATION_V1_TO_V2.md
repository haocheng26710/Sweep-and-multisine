...
DEV-C14 validation creates actual simulated P7 stimuli, S3 recordings with known affine injection, P8 estimates and persisted P3-C FeatureSets before invoking P9-C. Its lifecycle is fixed to `software_validation_only`, approval is `not_approved`, and `scientifically_eligible`, `deployment_eligible`, `canonical_analysis` and `final_test_read` are false. No simulated model can be migrated into a real calibration or scientific conclusion. P9-D deployment, real calibration approval, uncertainty qualification and real threshold freezing remain outside this migration.
