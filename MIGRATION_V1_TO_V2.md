...
P6-B phase remains `magnitude_only`; uncertainty remains `unavailable` without an authoritative uncertainty source. `deployment_allowed=false` and `absolute_energy_comparable=false` are fixed for this slice. DEV-C11 validation is simulated/software-validation/scientifically ineligible and cannot support scientific conclusions. P9, cross-mode amplitude calibration, real HR threshold freezing, and deployment remain outside this migration.
