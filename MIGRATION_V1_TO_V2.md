...
## DEV-UI4 frozen-delivery migration boundary

DEV-UI4 does not alter the pipeline/config/Measurement/FeatureSet version quartet, scientific algorithms, thresholds or existing CLI arguments. It adds a desktop application-service layer and `--worker` dispatcher so source and PyInstaller-frozen processes call the same existing P1/P2/P4/P5/P6/P9/acceptance functions without external Python or `scripts/*.py` assumptions.

Packaged resources are read-only; sessions, plans, staging, logs and outputs use an explicit writable workspace. Existing source defaults remain compatible. P9 scopes/manifests remain the formal backend schemas and are always selected explicitly; no V1 filenames or directories are mined for authority. P9-D UI evidence (`freeze_approval_record.json`, `package_validation.json`, reports and checksums) is presentation/audit evidence, not a new model or scientific authority.

The final-test UI implements a fail-closed approval state machine but does not grant approval. In the current repository it remains sealed because real Multisine/P8 and complete real authorities are absent. Simulated/external-reference packages cannot be migrated or relabelled into real approved packages. Existing V1/V2 CLI users may continue unchanged; Windows users may optionally use the one-folder build documented in the Chinese guide.
