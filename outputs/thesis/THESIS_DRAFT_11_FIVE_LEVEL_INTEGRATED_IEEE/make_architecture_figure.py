...
def setup_head(ax, title, zh=False):
    ...
    ax.set_title(title, fontsize=9.0, pad=2, **font_kwargs(zh))
...
def add_orientation_and_scale(ax, zh=False):
    ...
    ax.text(-87, -124, "50 mm", ha="center", va="bottom", fontsize=7.2)


def add_outcome(ax, text, zh=False):
    """Place one high-level result inside each panel without obscuring geometry."""
    ax.text(16, -111, text, ha="center", va="center", fontsize=7.1,
...
def draw_integrated(ax, zh=False):
    ...
    ax.set_title("(d) 一体化相对双通道装置" if zh else "(d) Integrated opposed two-channel device",
                 fontsize=9.0, pad=2, **font_kwargs(zh))
    ...
    ax.text(24, 61, "北通道 1.85 kHz" if zh else "north channel 1.85 kHz",
            ha="left", va="center", fontsize=7.4, color="#222222", **font_kwargs(zh))
    ...
    ax.text(42, 0, "麦克风微型汇合腔" if zh else "microphone microplenum",
            ha="left", va="center", fontsize=7.2, **font_kwargs(zh))
...
def generate(out_dir, zh=False):
    fig, axes = plt.subplots(2, 2, figsize=(7.15, 6.10), constrained_layout=True)
