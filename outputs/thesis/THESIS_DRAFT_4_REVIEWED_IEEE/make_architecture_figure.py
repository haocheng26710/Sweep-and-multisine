...
def setup_head(ax, title, zh=False):
    ...
    ax.set_title(title, fontsize=8.3, pad=2, **font_kwargs(zh))
...
def draw_dummy(ax, angle):
    # P09 occupies the fixed passages; it is not an open acoustic branch.
    add_polygon(ax, radial_polygon(17.0, 105.0, 7.4, 15.0, angle), DUMMY,
                edge="#757d85", lw=0.30, z=2, hatch="xx")
...
def label_cardinal(ax, angle, text, zh=False, radius=77, color="#222222"):
    ...
    ax.text(x, y, text, ha="center", va="center", fontsize=6.15, color=color,
            bbox=dict(boxstyle="round,pad=0.12", fc="white", ec="none", alpha=0.82),
            zorder=10, **font_kwargs(zh))


def add_orientation_and_scale(ax, zh=False):
    ...
    ax.text(-108, 112, "北" if zh else "N", ha="center", va="bottom",
            fontsize=6.2, **font_kwargs(zh))
    ax.plot([-112, -62], [-118, -118], color="#222222", lw=1.1, zorder=12)
    ax.plot([-112, -112], [-121, -115], color="#222222", lw=0.7, zorder=12)
    ax.plot([-62, -62], [-121, -115], color="#222222", lw=0.7, zorder=12)
    ax.text(-87, -113, "50 mm", ha="center", va="bottom", fontsize=5.8)
...
def draw_reference(ax, zh=False):
    ...
    add_orientation_and_scale(ax, zh)
...
def draw_integrated(ax, zh=False):
    ...
    ax.set_title("(d) 一体化相对双通道装置" if zh else "(d) Integrated opposed two-channel device",
                 fontsize=8.3, pad=2, **font_kwargs(zh))
    ...
    ax.text(24, 61, "北通道 1.85 kHz" if zh else "north channel 1.85 kHz",
            ha="left", va="center", fontsize=6.25, color="#222222", **font_kwargs(zh))
    ...
    ax.text(42, 0, "麦克风微型汇合腔" if zh else "microphone microplenum",
            ha="left", va="center", fontsize=6.0, **font_kwargs(zh))
    add_orientation_and_scale(ax, zh)


def generate(out_dir, zh=False):
    fig, axes = plt.subplots(2, 2, figsize=(7.15, 6.55), constrained_layout=True)
