"""Generate the bilingual Draft-15 physical-workflow and full-wave figures.
...
def draw_setup(ax, zh=False):
    ...
    panel_label(ax, "(a)", zh)
    ax.plot([0.45, 9.55], [0.75, 0.75], color="#707982", lw=0.85)
    draw_speaker(ax, 1.75, 3.18)
    draw_device_top(ax, 7.55, 3.18, 0.86)
    tx(ax, 1.75, 2.18, "Logitech Z207", "Logitech Z207", zh, ha="center", va="top", fontsize=8.0, color=INK)
    ax.annotate("", xy=(6.62, 3.18), xytext=(2.48, 3.18), arrowprops=dict(arrowstyle="-|>", lw=1.25, color=GOLD))
    tx(ax, 4.55, 3.38, "incident sweep", "入射扫频", zh, ha="center", va="bottom", fontsize=8.0, color="#75500f")
    ax.annotate("", xy=(6.77, 1.62), xytext=(2.46, 1.62), arrowprops=dict(arrowstyle="<->", lw=0.75, color=MUTED))
    tx(ax, 4.62, 1.47, "0.8 m between acoustic centres", "声学中心间距 0.8 m", zh, ha="center", va="top", fontsize=7.9, color=MUTED)
    ax.annotate("", xy=(0.68, 3.18), xytext=(0.68, 0.75), arrowprops=dict(arrowstyle="<->", lw=0.75, color=MUTED))
    tx(ax, 0.48, 1.97, "0.8 m", "0.8 m", zh, rotation=90, ha="center", va="center", fontsize=8.0, color=MUTED)
    for dx, dy, lab in ((0, 1.17, "N"), (1.14, 0, "E"), (0, -1.17, "S"), (-1.14, 0, "W")):
        scale = 1.12 if lab in ("N", "S") else 1.27
        tx(ax, 7.55 + dx*scale, 3.18 + dy*scale, lab, {"N": "北", "E": "东", "S": "南", "W": "西"}[lab], zh,
           ha="center", va="center", fontsize=7.8, color=MUTED)
    ax.add_patch(Arc((7.55, 3.18), 2.35, 2.35, theta1=-70, theta2=28, lw=0.8, color=MUTED))
    ax.annotate("", xy=(8.60, 3.70), xytext=(8.43, 3.92), arrowprops=dict(arrowstyle="-|>", lw=0.8, color=MUTED))
    tx(ax, 8.62, 4.73, "indexed rotation", "索引旋转", zh, ha="center", va="center", fontsize=7.6, color=MUTED)
    ax.annotate("", xy=(7.55, 2.98), xytext=(8.35, 1.18), arrowprops=dict(arrowstyle="->", lw=0.75, color=RED))
    ax.add_patch(FancyBboxPatch((7.70, 0.57), 1.62, 0.78, boxstyle="round,pad=0.05", fc="white", ec="#818a93", lw=0.7))
    tx(ax, 8.51, 0.96, "central mic → REW\n48 kHz", "中央麦克风 → REW\n48 kHz", zh,
       ha="center", va="center", fontsize=7.6, color=INK, linespacing=1.1)
...
def draw_small_head(ax, mode, zh=False):
    ...
    if mode == "straight":
        title = ("Straight reference", "直通参考")
        result = ("shared-junction baseline", "共享汇合基线")
    elif mode == "heterogeneous":
        title = ("Heterogeneous paths", "异质通道")
        result = ("held-block BA = 0.250", "留块 BA = 0.250")
    else:
        title = ("Four assigned bands", "四个预设频带")
        result = ("0/4 bands ranked first", "0/4 频带排名第一")
    tx(ax, 0, 1.24, *title, zh, ha="center", va="top", fontsize=8.1, fontweight="bold", color=INK)
    tx(ax, 0, -1.15, *result, zh, ha="center", va="bottom", fontsize=7.6, color="#6b4b0e",
       bbox=dict(boxstyle="round,pad=0.13", fc="#fff5d8", ec="#cba44f", lw=0.5))
...
def draw_integrated(ax, zh=False):
    ...
    ax.annotate("", xy=(0.18, 0), xytext=(0.83, 0), arrowprops=dict(arrowstyle="->", lw=0.7, color=MUTED))
    tx(ax, 0.92, 0, "microplenum", "微型汇合腔", zh, ha="left", va="center", fontsize=7.3, color=MUTED)
    tx(ax, 0, 1.24, "Opposed two-channel", "相对双通道", zh, ha="center", va="top", fontsize=8.1,
       fontweight="bold", color=INK)
    tx(ax, 0, -1.15, "N–S 2.069 dB > return 1.131 dB", "南北 2.069 dB > 回返 1.131 dB", zh,
       ha="center", va="bottom", fontsize=7.45, color="#6b4b0e",
       bbox=dict(boxstyle="round,pad=0.13", fc="#fff5d8", ec="#cba44f", lw=0.5))


def physical_figure(out_dir: Path, zh=False):
    fig = plt.figure(figsize=(7.16, 4.85), constrained_layout=True, facecolor="white")
    gs = fig.add_gridspec(2, 12, height_ratios=(1.0, 1.12))
...
def fullwave_figure(out_dir: Path, zh=False):
    ...
    tx(axes[1], 0.05, 0.10, "one stored boundary excitation", "一个已存储边界激励", zh,
