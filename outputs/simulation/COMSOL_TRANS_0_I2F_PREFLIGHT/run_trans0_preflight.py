"""TRANS-0 COMSOL 6.4 geometry/interface preflight for TRANS-I2F V1.
...
def build_model(client: mph.Client, kind: str, excitation: str, mesh_size: int,
    ...
    selection_audit = {
        ...
        "identity_checks": {
            ...
            "expected_port_area_m2": 20.0e-3 * AIR_H,
            "port_area_relative_error_max": max(abs(entity_area(geom, nport)/(20.0e-3*AIR_H)-1), abs(entity_area(geom, sport)/(20.0e-3*AIR_H)-1)),
    ...
    if not (selection_audit["all_required_nonempty"] and selection_audit["walls_all_exterior"]
            ...
            and selection_audit["ports_excluded_from_walls"] and connectivity["connected"]):
        raise RuntimeError({"selection_audit": selection_audit, "connectivity": connectivity})
    ...
    return model, {
        ...
        "selection_audit": selection_audit, "boundary_adjacency": adjacency,
...
def run_smoke(client: mph.Client) -> dict[str, Any]:
    model, audit = build_model(client, "ISO_CODED", "N", 7, SMOKE_FREQ, "TRANS0_PREFLIGHT_ISO_CODED_N")
    start = time.time()
    ...
    path = OUT / "TRANS0_PREFLIGHT_ISO_CODED_N.mph"
    model.save(path)
    ...
    result = {"success": True, "elapsed_s": time.time()-start, "frequencies_hz": values["frequency"].tolist(),
              ...
              "mph": path.name, "mph_sha256": sha256(path), "reload": reload_validation, **audit}
