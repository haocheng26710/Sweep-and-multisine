"""TRANS-1 RETRY_01 bounded internal selectivity simulation.

Geometry, selections, physics, and Java study execution are delegated to the
accepted TRANS-0 RETRY_02 implementation.  This runner adds only the frozen
coarse/fine numerical gate, six formal combinations, reload verification, and
deterministic fixed-window metrics.  It never reads final-test data.
"""
...
def solve_case(client: mph.Client, kind: str, excitation: str, frequencies: np.ndarray,
    ...
    try:
        ...
        values = R0.evaluate(model, frequencies.size)
        validate_values(values, frequencies)
        ...
        reloaded = R0.evaluate(loaded, frequencies.size)
        validate_values(reloaded, frequencies)
        field_differences = {
...
def run_all(client: mph.Client, server_port: int) -> dict[str, Any]:
    ...
    try:
        ...
        atomic_json(OUT / "solver_session_license_log.json", session_log)
        atomic_json(OUT / "execution_state.json", {
            "technical_status": "TRANS1_RETRY_01_BLOCKED_BY_SOLVER",
            "numerical_gate_pass": False, "formal_six_completed": False,
