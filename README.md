...
The one permitted pre-pilot revision is frozen in [RESEARCH_PROTOCOL_REV002.md](docs/experiment/RESEARCH_PROTOCOL_REV002.md), with the complete 96-sweep preregistered matrix and acquisition order in [FORMAL_ACQUISITION_PLAN_REV001.md](docs/experiment/FORMAL_ACQUISITION_PLAN_REV001.md). The subsequently supplied streamlined real Sweep archive contains a human-frozen 72 ACTIVE/19 EXCLUDED subset; it does not retroactively complete the 96-sweep matrix. Real Multisine/P8 remains closed and final-test remains sealed.
...
FORMAL-2 prepares the empty, repository-external formal acquisition package and fail-closed B01 preflight gate. It derives the 96-row manifest and B01 sheet only from the frozen acquisition plan, creates blank geometry/environment/session/deviation/metadata templates, and hash-audits every baseline artifact without reading measurements. At the FORMAL-2 baseline, external backup was deliberately `not_configured`, `CMM29939.txt` was missing, all human checks were unsigned, and therefore `ready_for_B01=false`. See [FORMAL-2 preflight report](docs/progress/FORMAL_2_PREFLIGHT_PACKAGE.md).
...
SUP-1/SUP-2R now adds fixed-window mechanism localization using only existing real data. The A–H single-entry library uses the 29 already acquired SUP-1 measurements; HOM-A is the existing U4SYM AS01 B01/B02 data and HET-ABDF is the existing U4ENC AS01 B03/B04 data, so neither array was reacquired. The new result localizes HET−HOM differences and compares them with A/B/D/F single-entry signatures while preserving FORMAL-4/5 as the overall authority: only the 270°–ENC-F mapped correspondence is stable across the primary and outlier-sensitivity views, and independent module contributions remain unidentifiable. See the [SUP-1 module library](docs/progress/SUP_1_SINGLE_ENTRY_MODULE_LIBRARY.md) and [SUP-2R existing-data reanalysis](docs/progress/SUP_2R_EXISTING_DATA_MECHANISM_REANALYSIS.md).

The V2.5 S1–S3 activity adds 86 real TXT measurements and their 86 preserved MDAT counterparts: 54 single-entry P05/HR calibration sweeps, 16 U4SYM direction sweeps and 16 U4HR direction sweeps. Under the user's revised single-campaign 5-of-6 / 3-of-4 rule, U4HR shows a larger direction-related spectral effect than U4SYM, but the frozen HR01/HR03/HR05/HR07 diagonal direction-code mapping is not supported. The result therefore motivates no-print common-cavity/multi-port coupling simulation before distance changes or another tuned-module print. See the [V2.5 joint results](docs/progress/V25_S1_S2_S3_JOINT_RESULTS.md).

The TRANS-1 compact two-port physical pilot adds 15 real REW measurements and their 15 preserved MDAT counterparts in an N×6 → S×6 → N-return×3 sequence. With all repeats retained, the 200–4000 Hz demeaned N−S spectral effect is 2.069 dB, 2.165 times the current within-condition p95, while N-return−S reproduces the effect at 2.079 dB with 0.851 curve correlation. The predeclared 1850/3800 Hz low-minus-high contrast is 2.753 dB (95% CI 2.178–3.094). This supports a two-state magnitude-spectrum code within one closure, not four-direction localisation, cross-assembly generalisation or exact COMSOL peak prediction. See the [TRANS-1 physical-pilot results](docs/progress/TRANS1_TWO_PORT_PHYSICAL_PILOT_RESULTS.md).

The dissertation writing set now includes the [final integrated experimental chapters](docs/dissertation/EXPERIMENTAL_CHAPTERS_FINAL.md), [citation requirements](docs/dissertation/FINAL_CITATION_REQUIREMENTS.md), and [final consistency audit](docs/dissertation/FINAL_CONSISTENCY_AUDIT.md). The component [Results](docs/dissertation/RESULTS_DRAFT.md), [Methods](docs/dissertation/METHODS_DRAFT.md), [Discussion](docs/dissertation/DISCUSSION_DRAFT.md), and [Limitations and Future Work](docs/dissertation/LIMITATIONS_AND_FUTURE_WORK.md) drafts remain as traceable inputs. Figures and tables remain bound to the [WRITE-2 manifest](docs/dissertation/FIGURE_AND_TABLE_MANIFEST.md); WRITE-4 did not reopen analysis or final-test.
...
DEV-UI3 retains the twelve ordered step cards, default simple mode and professional details. In addition to DEV-UI1/2 validation and single-measurement actions, users can create immutable plan revisions and expected sample matrices, save the DEV-D safety checklist, explicitly register UI2 sessions/outputs, inspect metadata/hash match states, preview formal P2-B scope/input artifacts, and run supported P2-B/P4/P5/P6 formal CLIs in the background. Plans live under `outputs/ui_plans/`, registry evidence under `outputs/ui3_registry/`, and batch previews under `outputs/ui3_runs/`; all are ignored outputs and never replace backend authority.
...
## P9-D frozen offline fast readout

P9-D separates package construction from inference. Package construction consumes only explicitly listed training/development FeatureSets and exact P2-B/P4-B/P5/P9 authority snapshots. It freezes a complete nearest-centroid model, preprocessing/QC policy, exact P7 stimulus/tone contract, training membership and hashes into an immutable `FrozenReadoutPackage`. Temporary CV-fold models, final-test inputs, simulated artifacts relabelled as real, and unapproved P9-C models fail closed.

Offline inference consumes only persisted P8 `SpectrumData`, persisted P2 single-measurement QC, the exact P7 stimulus manifest and a verified package. It calls the existing P3-C multisine tone FeatureSet builder, optionally applies only an explicitly approved `frozen_readout` P9-C calibration, and then uses the shared frozen direction predictor. It never reads WAV/TXT, generates P7, reruns P8, trains a model, scans directories, interpolates missing tones or fills them with zero.

Build one package from explicit persisted authorities:

```powershell
python scripts/build_readout_package.py --config <config.yaml> --manifest <package_training_manifest.json> --output-dir <new_package_dir>
```

Run one persisted offline measurement:

```powershell
python scripts/run_offline_readout.py --config <config.yaml> --package-dir <package_dir> --input-manifest <readout_input_manifest.json> --output-dir <new_output_dir>
```
...
```powershell
$env:PYTHONPATH='src'
python scripts/run_offline_readout_validation.py --project-root . --config config/validation_dev_c15_p9d.yaml --output-root outputs --run-id <run-id>
```
