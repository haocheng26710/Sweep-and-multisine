...
The confirmation-stage research question, primary `G_demeaned` metric, data/QC thresholds, grouped-classification rules, decision categories and revision policy are frozen in [RESEARCH_QUESTION_AND_DECISION_RULES.md](docs/experiment/RESEARCH_QUESTION_AND_DECISION_RULES.md) as `rev-001`. The six-scenario, 50-seed execution boundary is frozen in [BOUNDED_STRESS_SIMULATION_PLAN.md](docs/experiment/BOUNDED_STRESS_SIMULATION_PLAN.md). SIM-1 has not started; current or future simulated software-validation data cannot support scientific conclusions or tune the frozen success thresholds.
