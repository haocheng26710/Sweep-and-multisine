...
DEV-UI2 retains the twelve ordered step cards, default simple mode and professional details. In addition to DEV-UI1 environment/config/acceptance actions, “导入并检查单次数据” now supports four fail-closed paths: official REW reference validation, real REW diagnostic registration, simulated Multisine/P8 validation, and real Multisine registration without analysis. It reuses the existing REW parser, `MeasurementMeta` schema, configuration loader and mode-specific pipeline CLIs. UI-owned drafts and reports live under `outputs/ui_sessions/<sample-id>/rev-NNN/`; raw inputs remain read-only, every input is hash checked again before launch, output collisions are rejected, and correction creates a new revision. `data_origin`, `dataset_role`, `run_purpose`, `source_format` and scientific eligibility are route-derived rather than user-editable.

The real Multisine message is a hard gate, not a warning: the recording, manifest, sidecar and hashes may be registered, but the UI will not launch P8 until the DEV-D real-data gate is implemented and approved. DEV-UI3/DEV-UI4 dataset, analysis, freeze/final-test and complete reporting actions remain unavailable placeholders.
