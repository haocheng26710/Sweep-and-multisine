...
from scripts.gen_enc_b1_repair_08.primitives import canonical, sha_file, sha_value
from tests.helpers.gen_enc_b1_repair_08_control import authorize_phase_b, setup
...
def phase_a(root:Path)->dict:
    c=setup(root);assert run(c,"scripts.gen_enc_b1_repair_08.formal_driver","preflight").returncode==0;assert run(c,"scripts.gen_enc_b1_repair_08.formal_driver","generate-staging").returncode==0
    verified=run(c,"scripts.gen_enc_b1_repair_08.independent_verifier","verify");assert verified.returncode==0,verified.stderr
    return c
...
def test_phase_a_cannot_publish_and_old_six_command_release_is_rejected(tmp_path:Path)->None:
    c=phase_a(tmp_path/"no-publish");result=run(c,"scripts.gen_enc_b1_repair_08.formal_driver","publish")
    ...
    denied=run(c2,"scripts.gen_enc_b1_repair_08.formal_driver","preflight");assert denied.returncode==2 and "PHASE_A_RELEASE_CONTRACT" in denied.stderr and '"authority_read_count": 0' in denied.stderr
...
def test_all_phase_b_commands_reject_missing_external_expected_values(tmp_path:Path)->None:
    ...
    for command in driver.PHASE_B_COMMANDS:
        result=run(c,"scripts.gen_enc_b1_repair_08.formal_driver",command);assert result.returncode==2 and "EXTERNAL_EXPECTED_REQUIRED" in result.stderr
    assert not c["roots"]["run"].exists() and not c["roots"]["pointer"].exists() and not c["roots"]["success"].exists()
...
def test_guardian_common_contract_and_review_bytes_are_pre_read_bound(tmp_path:Path)->None:
    for index,rel in enumerate((driver.GUARDIAN_TWO_PHASE_CONTRACT_PATH,driver.GUARDIAN_TWO_PHASE_REVIEW_PATH)):
        ...
        result=run(c,"scripts.gen_enc_b1_repair_08.formal_driver","preflight");assert result.returncode==2 and "GUARDIAN_TWO_PHASE_AUTHORITY_BYTES" in result.stderr and '"authority_read_count": 0' in result.stderr


def test_repair08_entrypoints_are_independent_and_have_no_dynamic_imports()->None:
    for name in ("formal_driver.py","independent_verifier.py","cad_projection_driver.py","cad_projection_verifier.py"):
        tree=ast.parse((REPO/"scripts/gen_enc_b1_repair_08"/name).read_text())
...
def test_phase_b_full_chain_requires_external_expected_hashes(tmp_path:Path)->None:
    c,b=phase_b_ready(tmp_path/"full");published=run(c,"scripts.gen_enc_b1_repair_08.formal_driver","publish",b);assert published.returncode==0,published.stderr
    packaged=run(c,"scripts.gen_enc_b1_repair_08.formal_driver","package-results",b);assert packaged.returncode==0,packaged.stderr
...
def test_phase_b_external_and_binding_attacks_fail_zero_publication(attack:str,tmp_path:Path)->None:
    ...
    result=run(c,"scripts.gen_enc_b1_repair_08.formal_driver","publish",b)
...
def test_coordinated_stage_seal_report_replacement_cannot_match_guardian_hash(tmp_path:Path)->None:
    ...
    result=run(c,"scripts.gen_enc_b1_repair_08.formal_driver","publish",b);assert result.returncode==2 and not c["roots"]["run"].exists() and not (c["roots"]["pointer"]/"B1.json").exists()


def test_wrong_phase_b_path_and_reused_dispatch_are_rejected(tmp_path:Path)->None:
    c,b=phase_b_ready(tmp_path/"path");args=argv("scripts.gen_enc_b1_repair_08.formal_driver","publish",b);args[args.index(driver.PHASE_B_AUTH_PATH)]="control/not-guardian.json";wrong=subprocess.run(args,cwd=c["root"],env=env(),text=True,capture_output=True,check=False);assert wrong.returncode==2 and "EXTERNAL_EXPECTED_REQUIRED" in wrong.stderr
    assert run(c,"scripts.gen_enc_b1_repair_08.formal_driver","publish",b).returncode==0
    reused=run(c,"scripts.gen_enc_b1_repair_08.formal_driver","publish",b);assert reused.returncode==2 and "PHASE_B_ALREADY_CONSUMED" in reused.stderr
...
def test_phase_b_required_and_zero_unlisted_fields(record_name:str,attack:str,tmp_path:Path)->None:
    ...
    _rewrite_phase_b_chain(c,b,mutate);result=run(c,"scripts.gen_enc_b1_repair_08.formal_driver","publish",b)
...
def test_b1_r06_b02_phase_a_failure_is_permanent_for_same_run(tmp_path:Path)->None:
    c=setup(tmp_path/"phase-a-permanent");assert run(c,"scripts.gen_enc_b1_repair_08.formal_driver","preflight").returncode==0;assert run(c,"scripts.gen_enc_b1_repair_08.formal_driver","generate-staging").returncode==0
    ...
    failed=run(c,"scripts.gen_enc_b1_repair_08.independent_verifier","verify");assert failed.returncode==2
    ...
    retried=run(c,"scripts.gen_enc_b1_repair_08.independent_verifier","verify")
...
def test_phase_b_rejects_mixed_handoff_and_phase_a_failure_before_copy(tmp_path:Path)->None:
    ...
    b=authorize_phase_b(c);result=run(c,"scripts.gen_enc_b1_repair_08.formal_driver","publish",b)
...
def test_phase_b_postcopy_exception_rolls_back_pointer_and_run_to_unique_failure(tmp_path:Path)->None:
    c,b=phase_b_ready(tmp_path/"postcopy");result=run(c,"scripts.gen_enc_b1_repair_08.formal_driver","publish",b,{"GEN_ENC_B1_REPAIR08_FAULT":"post-copy-exception"})
...
def test_phase_b_consumed_runtime_revalidates_complete_common_binding(tmp_path:Path)->None:
    c,b=phase_b_ready(tmp_path/"consumed");published=run(c,"scripts.gen_enc_b1_repair_08.formal_driver","publish",b);assert published.returncode==0,published.stderr
    ...
    packaged=run(c,"scripts.gen_enc_b1_repair_08.formal_driver","package-results",b)
...
def test_phase_a_dispatch_eight_process_one_use(tmp_path:Path)->None:
    c=setup(tmp_path/"race-a");processes=[subprocess.Popen(argv("scripts.gen_enc_b1_repair_08.formal_driver","preflight"),cwd=c["root"],env=env(),stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True) for _ in range(8)];results=[p.communicate()+(p.returncode,) for p in processes]
    assert sum(code==0 for _,_,code in results)==1


def test_phase_b_dispatch_eight_process_one_use(tmp_path:Path)->None:
    c,b=phase_b_ready(tmp_path/"race-b");processes=[subprocess.Popen(argv("scripts.gen_enc_b1_repair_08.formal_driver","publish",b),cwd=c["root"],env=env(),stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True) for _ in range(8)];results=[p.communicate()+(p.returncode,) for p in processes]
    assert sum(code==0 for _,_,code in results)==1


@pytest.mark.parametrize("index",range(20))
def test_phase_b_real_kill_each_target_then_recover(index:int,tmp_path:Path)->None:
    c,b=phase_b_ready(tmp_path/f"kill-{index}");crash=run(c,"scripts.gen_enc_b1_repair_08.formal_driver","publish",b,{"GEN_ENC_B1_REPAIR08_FAULT":f"kill:{index}"});assert crash.returncode==86
    recovered=run(c,"scripts.gen_enc_b1_repair_08.formal_driver","recover",b);assert recovered.returncode==0,recovered.stderr
...
def test_phase_b_commit_boundaries(boundary:str,code:int,success:bool,tmp_path:Path)->None:
    c,b=phase_b_ready(tmp_path/boundary);crash=run(c,"scripts.gen_enc_b1_repair_08.formal_driver","publish",b,{"GEN_ENC_B1_REPAIR08_FAULT":boundary});assert crash.returncode==code
    recovered=run(c,"scripts.gen_enc_b1_repair_08.formal_driver","recover",b);assert recovered.returncode==0,recovered.stderr
