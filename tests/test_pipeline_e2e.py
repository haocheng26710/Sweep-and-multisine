...
def test_clean_multisine_run_writes_reproducible_isolated_output_bundle(
    ...
    assert manifest["run_manifest_schema_version"] == "1.15.0"
    assert manifest["qc_schema_version"] == "1.0.0"
    assert manifest["versions"] == {
        "pipeline": "2.0.0-dev.21",
        "config_schema": "2.20.0",
    ...
    assert manifest["stage_gate"]["P5_B"] == "cross_mode_classification_scope_required"
    assert manifest["stage_gate"]["P6_A"] == "hr_calibration_scope_required"
    ...
    assert manifest["stage_gate"]["P9_D"] == "frozen_readout_package_required"
