...
from scripts.gen_enc_b1_repair_11.build_freeze_package import PACKAGE, build
...
def make_case(objects:dict[str,Any]|None=None,fail_driver:int|None=None,fail_verifier:int|None=None,read_delay_ms:int=0)->dict[str,Any]:
    build()
    repo=Path.cwd();tag=uuid.uuid4().hex;base=Path(".r11t")/tag;control=base/"control";run_id=hashlib.sha256(("run:"+tag).encode()).hexdigest();dispatch_id=hashlib.sha256(("dispatch:"+tag).encode()).hexdigest()
    ...
    roots={name:(Path(".r11t")/run_id/"shape_run"/name).as_posix() for name in ("staging","receipts","side_records","package","terminal")}
    ...
    release={"schema_version":"gen_enc_b1_r11_shape_release_v1","record_kind":"SHAPE_ONLY_RELEASE","task_id":TASK,"batch_id":"B1","run_id":run_id,"dispatch_id":dispatch_id,"mode":MODE,"commands_exact":COMMANDS,**{key+"_path":path.as_posix() for key,path in files.items()},**{key+"_sha256":sha(path.read_bytes()) for key,path in files.items()},"guardian_contract_path":"outputs/governance/RESEARCH_SCOPE_CONTRACTION_AUDIT/contracts/GEN_ENC_FAST_B1_REPAIR_11_AUTHORITY_SHAPE_ONLY_CONFORMANCE_CONTRACT.json","guardian_contract_sha256":"944ecf3d7b7307d1f2df4ff9fbd887bf64896bb8393a88e8f2dba5fd0328cb7b","roots":roots,"final_test_read":False,"release_path":release_path,"attestation_path":attestation_path,"dispatch_path":dispatch_path,"permissions":{"shape_preflight":True,"extract_shape":True,"verify_shape":True,"package_shape":True,"identity":False,"static":False,"publication":False,"phase_b":False},"technical_mirror":True,"technical_bundle_path":bundle_path.as_posix(),"technical_bundle_sha256":sha((repo/bundle_path).read_bytes()),"technical_fail_after_driver_read":fail_driver,"technical_fail_after_verifier_read":fail_verifier,"technical_read_delay_ms":read_delay_ms}
    ...
    att={"schema_version":"gen_enc_b1_r11_shape_attestation_v1","record_kind":"GUARDIAN_SHAPE_ONLY_ATTESTATION",**binding,"release_path":release_path,"release_sha256":rhash,"one_way":True};write(repo/attestation_path,att);ahash=sha((repo/attestation_path).read_bytes())
    dispatch={"schema_version":"gen_enc_b1_r11_shape_dispatch_v1","record_kind":"TASK_BOUND_SHAPE_DISPATCH",**binding,"release_path":release_path,"release_sha256":rhash,"attestation_path":attestation_path,"attestation_sha256":ahash,"state":"SHAPE_ISSUED","one_use":True,"revoked":False};write(repo/dispatch_path,dispatch)
...
def argv(case:dict[str,Any],command:str)->list[str]:
    ...
    return ["python","-m",module,command,"--repo-root",".","--release",case["release"].as_posix(),"--guardian-attestation",case["attestation"].as_posix(),"--dispatch",case["dispatch"].as_posix(),"--schema-root","schemas/gen_enc/b1_repair_11"]
