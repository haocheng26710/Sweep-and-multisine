...
def setup(root:Path,simulate_formal_reads:bool=False,fail_after_reads:int|None=None,fail_after_verifier_reads:int|None=None)->dict:
    root.mkdir(parents=True,exist_ok=True);(root/".technical_b1_repair_05_root").write_text("TECHNICAL ONLY\n")
    package=root/"outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_05";package.mkdir(parents=True)
    ...
    bind={k:release[k] for k in ("task_id","batch_id","run_id","mode","commands_exact","authority_allowlist_sha256","verification_subject_seal_path","projection_contract_path","projection_contract_sha256","dependency_matrix_path","dependency_matrix_sha256","dependency_structure_sha256","source_manifest_path","source_manifest_sha256","schema_manifest_path","schema_manifest_sha256","command_manifest_path","command_manifest_sha256","roots")}
    att={"schema_version":"gen_enc_fast_b1_repair_05_technical_attestation_v1","record_kind":"TECHNICAL_MIRROR_ONE_WAY_ATTESTATION","identity_class":"TECHNICAL_MIRROR_ONLY","repair_id":driver.REPAIR,**bind,"release_sha256":release_sha,"one_way":True,"final_test_read":False};write(root/"control/GUARDIAN_ATTESTATION.json",att);att_sha=sha_file(root/"control/GUARDIAN_ATTESTATION.json")
    issued={"schema_version":"gen_enc_fast_b1_repair_05_side_record_v1","state":"ISSUED","repair_id":driver.REPAIR,**bind,"release_sha256":release_sha,"attestation_sha256":att_sha,"one_use":True,"revoked":False,"final_test_read":False};write(root/release["roots"]["side_records"]/"ISSUED.json",issued)
