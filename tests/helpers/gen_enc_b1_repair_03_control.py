...
from scripts.gen_enc_b1_repair_03.primitives import canonical, sha_file
...
def setup(root:Path)->dict:
    root.mkdir(parents=True,exist_ok=True);(root/".technical_b1_repair_03_root").write_text("TECHNICAL ONLY\n")
    package=root/"outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_03";package.mkdir(parents=True)
    ...
    write(package/"command_manifest.json",json.loads((REPO/"outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_03/command_manifest.json").read_text()))
    release={"schema_version":"gen_enc_fast_b1_repair_03_technical_release_v1","record_kind":"TECHNICAL_MIRROR_RELEASE","identity_class":"TECHNICAL_MIRROR_RELEASE_NEVER_FORMAL","authoritative":False,"technical_only":True,"attempt":1,"repair_id":driver.REPAIR,"task_id":driver.TASK_ID,"batch_id":"B1","mode":driver.MODE,"commands_exact":list(driver.COMMANDS),"issued_at":datetime.now(timezone.utc).isoformat(),"expires_at":(datetime.now(timezone.utc)+timedelta(hours=2)).isoformat(),"run_id":"0"*64,"roots":driver.exact_roots("0"*64,True),"projection_contract_path":driver.PROJECTION_PATH,"projection_contract_sha256":sha_file(package/"cad_typed_projection_contract.json"),"dependency_matrix_path":driver.DEPENDENCY_PATH,"dependency_matrix_sha256":sha_file(package/"cad_leaf_dependency_matrix.json"),"dependency_structure_sha256":projected["dependency_matrix_sha256"],"source_manifest_path":"outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_03/source_manifest.json","source_manifest_sha256":sha_file(package/"source_manifest.json"),"schema_manifest_path":"outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_03/schema_manifest.json","schema_manifest_sha256":sha_file(package/"schema_manifest.json"),"command_manifest_path":"outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_03/command_manifest.json","command_manifest_sha256":sha_file(package/"command_manifest.json"),"technical_bundle_path":"technical/bundle.json","technical_bundle_sha256":sha_file(root/"technical/bundle.json"),"final_test_read":False}
    ...
    bind={k:release[k] for k in ("task_id","batch_id","run_id","mode","projection_contract_path","projection_contract_sha256","dependency_matrix_path","dependency_matrix_sha256","dependency_structure_sha256","source_manifest_path","source_manifest_sha256","schema_manifest_path","schema_manifest_sha256","command_manifest_path","command_manifest_sha256","roots")}
    att={"schema_version":"gen_enc_fast_b1_repair_03_technical_attestation_v1","record_kind":"TECHNICAL_MIRROR_ONE_WAY_ATTESTATION","identity_class":"TECHNICAL_MIRROR_ONLY","repair_id":driver.REPAIR,**bind,"release_sha256":release_sha,"one_way":True,"final_test_read":False};write(root/"control/GUARDIAN_ATTESTATION.json",att);att_sha=sha_file(root/"control/GUARDIAN_ATTESTATION.json")
    issued={"schema_version":"gen_enc_fast_b1_repair_03_side_record_v1","state":"ISSUED","repair_id":driver.REPAIR,**bind,"release_sha256":release_sha,"attestation_sha256":att_sha,"one_use":True,"revoked":False,"final_test_read":False};write(root/release["roots"]["side_records"]/"ISSUED.json",issued)
