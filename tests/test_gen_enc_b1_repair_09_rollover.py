...
from scripts.gen_enc_b1_repair_09 import independent_verifier as verifier
from scripts.gen_enc_b1_repair_09.primitives import PrimitiveError, canonical, sha_file
from tests.helpers.gen_enc_b1_repair_09_control import setup
...
def test_each_attestation_runtime_field_mutation_fails_both_pre_read_gates(field:str,tmp_path:Path)->None:
    ...
    with pytest.raises((driver.Repair09Error,KeyError)):driver.validate_control(c["root"],c["release"],c["attestation"],SCHEMAS,"generate-staging")
    with pytest.raises((verifier.Verify09Error,KeyError)):verifier.gate(c["root"],c["release"],c["attestation"],SCHEMAS)
...
def test_schema_valid_runtime_invalid_and_runtime_shape_valid_schema_invalid_are_both_rejected(tmp_path:Path)->None:
    ...
    with pytest.raises(driver.Repair09Error,match="ATTESTATION_BINDING"):driver.validate_control(c["root"],c["release"],c["attestation"],SCHEMAS,"preflight")
    ...
    with pytest.raises(driver.Repair09Error,match="SCHEMA_technical_guardian_attestation"):driver.validate_control(c["root"],c["release"],c["attestation"],SCHEMAS,"preflight")
...
def test_phase_a_dispatch_constants_and_structure_reject_in_both_entries(attack:str,tmp_path:Path)->None:
    ...
    with pytest.raises((driver.Repair09Error,PrimitiveError,ValueError,json.JSONDecodeError)):driver.validate_control(c["root"],c["release"],c["attestation"],SCHEMAS,"preflight")
    ...
    with pytest.raises((verifier.Verify09Error,PrimitiveError,ValueError,json.JSONDecodeError)):verifier.gate(c["root"],c["release"],c["attestation"],SCHEMAS)
...
def test_dynamic_advisor_subject_hash_mutation_fails_before_authority_read(subject:str,tmp_path:Path)->None:
    ...
    with pytest.raises((driver.Repair09Error,PrimitiveError)):driver.validate_control(c["root"],c["release"],c["attestation"],SCHEMAS,"generate-staging")
    with pytest.raises((verifier.Verify09Error,PrimitiveError)):verifier.gate(c["root"],c["release"],c["attestation"],SCHEMAS)
    assert driver.authority_read_count(d)==0


@pytest.mark.parametrize("path",[driver.ROLLOVER_SIGNING_FAIL_PATH,driver.ROLLOVER_REGISTRATION_PATH,driver.GUARDIAN_SIGNING_CHECK_PATH,driver.ADVISOR_B_FINAL_REVIEW_PATH,driver.ADVISOR_LEDGER_SNAPSHOT_PROTOCOL_PATH,driver.R07_LEDGER_SNAPSHOT_ABSENCE_PATH])
def test_each_fixed_signing_provenance_byte_mutation_fails_both_pre_read_gates(path:str,tmp_path:Path)->None:
    ...
    with pytest.raises((driver.Repair09Error,PrimitiveError)):driver.validate_control(c["root"],c["release"],c["attestation"],SCHEMAS,"preflight")
    with pytest.raises((verifier.Verify09Error,PrimitiveError)):verifier.gate(c["root"],c["release"],c["attestation"],SCHEMAS)
...
def test_dynamic_advisor_subject_wrong_root_is_rejected_without_discovery(subject:str,tmp_path:Path)->None:
    ...
    with pytest.raises(driver.Repair09Error,match="ADVISOR_DYNAMIC_ROOT"):driver._validate_signing_authority_bytes(c["root"],release,True)
    with pytest.raises(verifier.Verify09Error,match="ADVISOR_DYNAMIC_ROOT"):verifier._validate_signing_authority_bytes(c["root"],release,True)
...
def test_attestation_schema_required_set_is_runtime_binding_superset_and_no_discovery()->None:
    ...
    for path in (REPO/"scripts/gen_enc_b1_repair_09/formal_driver.py",REPO/"scripts/gen_enc_b1_repair_09/independent_verifier.py"):
...
def test_all_predecessor_control_paths_rejected_by_both_entrypoints_before_read(field:str,tmp_path:Path)->None:
    ...
    with pytest.raises(driver.Repair09Error,match="ROLLOVER_"):driver._validate_rollover_paths(release,c["release"],c["attestation"],c["dispatch"],c["root"],True)
    with pytest.raises(verifier.Verify09Error,match="ROLLOVER_"):verifier._validate_rollover_paths(release,c["release"],c["attestation"],c["dispatch"],c["root"],True)
    assert not c["roots"]["side_records"].exists()


@pytest.mark.parametrize("attack",["same_path","cross_run","casefold","traversal","absolute","revoked_run","revoked_dispatch"])
def test_rollover_path_run_and_dispatch_attacks_fail_both_independent_validators(attack:str,tmp_path:Path)->None:
    ...
    elif attack=="casefold":release["phase_a_release_path"]=release["phase_a_release_path"].replace("/R.","/r.")
    ...
    else:release["phase_a_dispatch_id"]=driver.REVOKED_DISPATCH_ID
    with pytest.raises(driver.Repair09Error,match="ROLLOVER_"):driver._validate_rollover_paths(release,c["release"],c["attestation"],c["dispatch"],c["root"],True)
    with pytest.raises(verifier.Verify09Error,match="ROLLOVER_"):verifier._validate_rollover_paths(release,c["release"],c["attestation"],c["dispatch"],c["root"],True)
...
def test_cross_run_control_mix_and_cli_dispatch_substitution_fail_pre_read(tmp_path:Path)->None:
    ...
    with pytest.raises(driver.Repair09Error,match="ROLLOVER_"):driver.validate_control(c1["root"],c1["release"],c2["attestation"],SCHEMAS,"preflight",c1["dispatch"])
    with pytest.raises(verifier.Verify09Error,match="ROLLOVER_"):verifier.gate(c1["root"],c1["release"],c2["attestation"],SCHEMAS)
    rel=lambda p:p.relative_to(c1["root"]).as_posix();wrong_dispatch=c2["dispatch"].relative_to(c2["root"]).as_posix();args=[sys.executable,"-m","scripts.gen_enc_b1_repair_09.formal_driver","preflight","--repo-root",".","--release",rel(c1["release"]),"--guardian-attestation",rel(c1["attestation"]),"--phase-a-dispatch",wrong_dispatch,"--schema-root",str(SCHEMAS),"--projection-contract",driver.PROJECTION_PATH,"--dependency-matrix",driver.DEPENDENCY_PATH];env=dict(os.environ);env["PYTHONPATH"]=str(REPO)+(os.pathsep+env["PYTHONPATH"] if env.get("PYTHONPATH") else "");result=subprocess.run(args,cwd=c1["root"],env=env,text=True,capture_output=True,check=False)
    assert result.returncode==2 and "ROLLOVER_DISPATCH_CLI_BINDING" in result.stderr and '"authority_read_count": 0' in result.stderr and not c1["roots"]["side_records"].exists()
...
def test_successor_paths_unique_run_bound_and_manifest_placeholder_mechanism(tmp_path:Path)->None:
    ...
    manifest=json.loads((REPO/"outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_09/command_manifest.json").read_text());placeholders={"<GUARDIAN_PHASE_A_RELEASE_EXACT_PATH>","<GUARDIAN_PHASE_A_ATTESTATION_EXACT_PATH>","<CONTROLLER_PHASE_A_DISPATCH_EXACT_PATH>"}
