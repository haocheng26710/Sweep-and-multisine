...
from scripts.gen_enc_b1_repair_11 import driver_core, verifier_core
from scripts.gen_enc_b1_repair_11.build_freeze_package import CONTRACT, CONTRACT_SHA, PACKAGE, SUPERVISION, SUPERVISION_SHA, build, sha_file
from tests.helpers.gen_enc_b1_repair_11_fixture import argv, canonical, make_case, synthetic_objects, write
...
def test_driver_verifier_import_independence_and_no_dynamic_import():
    forbidden_driver={"scripts.gen_enc_b1_repair_11.independent_verifier","scripts.gen_enc_b1_repair_11.verifier_core","scripts.gen_enc_b1_repair_10"}
    forbidden_verifier={"scripts.gen_enc_b1_repair_11.formal_driver","scripts.gen_enc_b1_repair_11.driver_core","scripts.gen_enc_b1_repair_10","tests.helpers"}
    for file,forbidden in ((Path("scripts/gen_enc_b1_repair_11/formal_driver.py"),forbidden_driver),(Path("scripts/gen_enc_b1_repair_11/independent_verifier.py"),forbidden_verifier)):
...
def test_full_four_command_cli_and_exact_26_receipts():
    ...
    receipts=sorted(case["roots"]["receipts"].glob("*.json"));assert len(receipts)==26
    values=[json.loads(p.read_text()) for p in receipts];assert [x["ordinal"] for x in values]==list(range(1,27));assert [x["lane"] for x in values]==["DRIVER"]*13+["VERIFIER"]*13
    terminal=json.loads((case["roots"]["terminal"]/"TERMINAL.json").read_text());assert terminal["status"]=="SUCCESS_SHAPE_ONLY_AWAITING_GUARDIAN_RESULT_SEAL";assert terminal["authority_read_count"]==26
...
def test_partial_driver_read_failure_is_honest_and_permanent():
    ...
    assert run(case,"extract-shape").returncode==2;assert len(list(case["roots"]["receipts"].glob("*.json")))==5
...
def test_real_subprocess_kill_preserves_partial_receipts_and_next_observation_seals_failure():
    ...
    while time.time()<deadline and len(list(case["roots"]["receipts"].glob("*.json")))<3:time.sleep(.01)
    process.kill();process.communicate(timeout=5)
    count=len(list(case["roots"]["receipts"].glob("*.json")));assert 1<=count<13;assert not (case["roots"]["staging"]/"driver_shape_snapshot.json").exists()
...
def test_control_extra_field_rejected_pre_read():
    case=make_case();value=json.loads(case["dispatch"].read_text());value["unexpected"]=1;write(case["dispatch"],value);r=run(case,"shape-preflight");assert r.returncode==2;assert not case["roots"]["receipts"].exists()
...
def test_cross_run_or_hash_binding_mutation_rejected_pre_read(field):
    case=make_case();value=json.loads(case["dispatch"].read_text());value[field]="0"*64;write(case["dispatch"],value);r=run(case,"shape-preflight");assert r.returncode==2;assert not case["roots"]["receipts"].exists()
...
def test_coordinated_arbitrary_root_rejected_by_both_runtime_templates():
    ...
    assert run(case,"shape-preflight").returncode==2;assert not case["roots"]["receipts"].exists()
...
def test_wrong_exact_argv_rejected_pre_read():
    case=make_case();bad=argv(case,"shape-preflight")+["--extra"];r=subprocess.run(bad,text=True,capture_output=True);assert r.returncode!=0;assert not case["roots"]["receipts"].exists()
...
def test_package_zero_unlisted_and_no_scientific_graph():
    ...
    assert run(case,"package-shape").returncode==2;terminal=json.loads((case["roots"]["terminal"]/"TERMINAL.json").read_text());assert terminal["status"]=="FAIL_CLOSED" and terminal["scientific_artifact_count"]==0
