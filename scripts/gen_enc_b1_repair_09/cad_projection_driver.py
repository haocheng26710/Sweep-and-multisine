...
from scripts.gen_enc_b1_repair_09.primitives import pointer, sha_value
...
def project(objects:Mapping[str,Any],entries_input:Sequence[Mapping[str,Any]],contract:Mapping[str,Any])->dict[str,Any]:
    ...
    for purpose in PURPOSES:
        ...
        # Canonical JSON deliberately sorts object keys.  The frozen object
        # shape is therefore an exact field set/cardinality contract, not an
        # insertion-order contract.
        if not isinstance(value,dict) or set(value)!=set(shape["field_names"]) or len(value)!=shape["field_count"]:raise ProjectionError("TOP_SHAPE:"+purpose)
    ...
    return {"schema_version":"gen_enc_fast_b1_repair_09_typed_cad_projection_v1","values":values,"semantic_leaf_count":len(matrix),"dependency_matrix":matrix,"dependency_matrix_sha256":sha_value(structural),"unused_leaf_allowlist":unused,"unused_leaf_count":len(unused),"projection_contract_sha256":sha_value(contract)}
