"""Standalone repair-03 driver: typed CAD values mechanically determine witnesses."""
...
from scripts.gen_enc_b1_repair_03.primitives import atomic_json, canonical, pointer, read_canonical, sha_bytes, sha_file, sha_value

TASK_ID="01a049a7-15ca-79e1-92a2-d3822ba8609d";BATCH_ID="B1";REPAIR="PRE-RELEASE-REPAIR-03"
...
RUN_DOMAIN=b"GEN-ENC-FAST-B1-PRE-RELEASE-REPAIR-03-RUN-ID-v1"
PROJECTION_PATH="outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_03/cad_typed_projection_contract.json"
DEPENDENCY_PATH="outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_03/cad_leaf_dependency_matrix.json"
MODE="FORMAL_B1_EXACT_20_TYPED_CAD";COMMANDS=("preflight","generate-staging","verify","publish","recover","package-results")


class Repair03Error(RuntimeError):pass
...
def _groups(spec:Mapping[str,Any],axes:Sequence[str],names:Sequence[str])->dict[str,tuple[float,float]]:
    if tuple(spec.get("parameter_order",()))!=tuple(axes) or spec.get("member_count")!=20 or spec.get("dof")!=len(axes):raise Repair03Error("SPEC_AXES")
    result={}
    for item,name in zip(spec.get("parameters",[]),names):
        if set(item)!={"group","axes","bounds"} or item["group"]!=name or len(item["bounds"])!=2:raise Repair03Error("SPEC_GROUP")
        lo,hi=map(float,item["bounds"])
        if not lo<hi:raise Repair03Error("SPEC_BOUND")
        for axis in item["axes"]:
            if axis in result:raise Repair03Error("DUP_AXIS")
            result[axis]=(lo,hi)
    if tuple(result)!=tuple(axes):raise Repair03Error("GROUP_AXIS_ORDER")
    return result
...
def parameters(objects:Mapping[str,Any],family:str,ordinal:int)->dict[str,float]:
    ...
    if family==FAMILIES[2]:
        ...
        if spec.get("uniform_algorithm")!={"interval":"OPEN_0_1_BINARY64","word_to_uniform":"((word>>11)+0.5)/2^53","one_guard":"nextafter(1.0,0.0)"} or spec.get("splitmix64",{}).get("gamma_hex")!="9e3779b97f4a7c15":raise Repair03Error("RANDOM_ALGORITHM")
    ...
    if spec.get("lhs_algorithm")!=["STRATA=20","SAMPLE=(PERMUTATION[row]+0.5)/20","JITTER=false","FISHER_YATES_SPLITMIX64=i19_TO_1"] or spec.get("lhs_master_seed")!=master:raise Repair03Error("PHYSICS_ALGORITHM")
    result={}
...
def derive_cad_evidence(family:str,params:Mapping[str,float],projection:Mapping[str,Any])->tuple[dict[str,Any],str]:
    ...
    for sector,weight in zip(SECTORS,weights):
        ...
        if abs(measured-target)>float(common["tolerance_m3"]):raise Repair03Error("ROOT_TOLERANCE")
...
def pre_read_binding(record:Mapping[str,Any],expected:Mapping[str,Any])->None:
    for key in ("task_id","batch_id","run_id","projection_contract_path","projection_contract_sha256","dependency_matrix_path","dependency_matrix_sha256","dependency_structure_sha256"):
        if record.get(key)!=expected.get(key):raise Repair03Error("PRE_READ_BINDING:"+key)


def _safe(repo:Path,value:str)->Path:
    path=(repo/value).resolve()
    if path==repo or not path.is_relative_to(repo):raise Repair03Error("PATH_CONTAINMENT")
    return path
...
def validate(value:Any,schemas:Mapping[str,Any],name:str)->None:
    ...
    except jsonschema.ValidationError as exc:raise Repair03Error("SCHEMA_"+name+":"+exc.message) from exc


def exact_roots(run_id:str,technical:bool=False)->dict[str,str]:
    base=(".technical_b1_repair_03/" if technical else "outputs/gen_enc/GEN_ENC_FAST_START/b1_repair_03_runs/")+run_id
    roots={x:f"{base}/{x}" for x in ("staging","run","journal","success","failure","side_records")};roots["pointer"]=(f".technical_b1_repair_03_commit/{run_id}" if technical else f"outputs/gen_enc/GEN_ENC_FAST_START/b1_repair_03_commit/{run_id}");return roots
...
def validate_control(repo:Path,release_path:Path,attestation_path:Path,schema_root:Path,command:str)->dict[str,Any]:
    ...
    if technical:
        if release.get("identity_class")!="TECHNICAL_MIRROR_RELEASE_NEVER_FORMAL" or not (repo/".technical_b1_repair_03_root").is_file():raise Repair03Error("TECHNICAL_ROOT_MARKER")
    else:validate(release,load_schemas(schema_root),"release")
    if release.get("record_kind") not in ("RELEASE","TECHNICAL_MIRROR_RELEASE") or release.get("repair_id")!=REPAIR or release.get("task_id")!=TASK_ID or release.get("batch_id")!=BATCH_ID or release.get("mode")!=MODE or release.get("commands_exact")!=list(COMMANDS) or command not in COMMANDS:raise Repair03Error("RELEASE_CONTRACT")
    if release["run_id"]!=derive_run_id(release):raise Repair03Error("RUN_ID")
    if datetime.fromisoformat(release["expires_at"]).astimezone(timezone.utc)<=datetime.now(timezone.utc):raise Repair03Error("EXPIRED")
    if release["roots"]!=exact_roots(release["run_id"],technical):raise Repair03Error("ROOT_TEMPLATE")
    roots={k:_safe(repo,v) for k,v in release["roots"].items()};vals=list(roots.values())
    if len({str(x).casefold() for x in vals})!=len(vals) or any(a.is_relative_to(b) or b.is_relative_to(a) for i,a in enumerate(vals) for b in vals[i+1:]):raise Repair03Error("ROOT_EXCLUSIVITY")
    ...
    if release["projection_contract_path"]!=PROJECTION_PATH or release["dependency_matrix_path"]!=DEPENDENCY_PATH:raise Repair03Error("PROJECTION_PATH")
    projection_path=_safe(repo,release["projection_contract_path"]);matrix_path=_safe(repo,release["dependency_matrix_path"])
    if sha_file(projection_path)!=release["projection_contract_sha256"] or sha_file(matrix_path)!=release["dependency_matrix_sha256"]:raise Repair03Error("PROJECTION_FILE_HASH")
    matrix,_=read_canonical(matrix_path)
    if matrix.get("dependency_structure_sha256")!=release["dependency_structure_sha256"] or matrix.get("projection_contract_sha256")!=release["projection_contract_sha256"]:raise Repair03Error("DEPENDENCY_STRUCTURE")
    for name in ("source","schema","command"):
        if sha_file(_safe(repo,release[f"{name}_manifest_path"]))!=release[f"{name}_manifest_sha256"]:raise Repair03Error("MANIFEST_HASH:"+name)
    command_manifest,_=read_canonical(_safe(repo,release["command_manifest_path"]))
    rows=command_manifest.get("entries",[])
    if command_manifest.get("mode")!=MODE or command_manifest.get("commands_exact")!=list(COMMANDS) or len(rows)!=6 or [row.get("command") for row in rows]!=list(COMMANDS):raise Repair03Error("COMMAND_MANIFEST")
    attestation,araw=read_canonical(attestation_path)
    if attestation.get("record_kind") not in ("GUARDIAN_ONE_WAY_ATTESTATION","TECHNICAL_MIRROR_ONE_WAY_ATTESTATION") or attestation.get("release_sha256")!=sha_bytes(raw) or attestation.get("one_way") is not True:raise Repair03Error("ATTESTATION")
    expected=_binding(release)
    for key,value in expected.items():
        if attestation.get(key)!=value:raise Repair03Error("ATTESTATION_BINDING:"+key)
    side=roots["side_records"];issued_path=side/"ISSUED.json";revoked=side/"REVOKED.json";consumed=side/"CONSUMED.json"
    if not issued_path.is_file() or revoked.exists():raise Repair03Error("ISSUED_REVOKED")
    issued,_=read_canonical(issued_path)
    if issued.get("state")!="ISSUED" or issued.get("revoked") is not False or issued.get("release_sha256")!=sha_bytes(raw) or issued.get("attestation_sha256")!=sha_bytes(araw):raise Repair03Error("ISSUED")
    for key,value in expected.items():
        if issued.get(key)!=value:raise Repair03Error("ISSUED_BINDING:"+key)
    if command=="preflight":
        if consumed.exists():raise Repair03Error("ALREADY_CONSUMED")
    else:
        if not consumed.is_file():raise Repair03Error("NOT_CONSUMED")
        consumed_value,_=read_canonical(consumed)
        if consumed_value.get("state")!="CONSUMED" or consumed_value.get("revoked") is not False:raise Repair03Error("CONSUMED")
        for key,value in {**expected,"release_sha256":sha_bytes(raw),"attestation_sha256":sha_bytes(araw)}.items():
            if consumed_value.get(key)!=value:raise Repair03Error("CONSUMED_BINDING:"+key)
    return {**expected,"technical":technical,"release":release,"release_sha256":sha_bytes(raw),"attestation_sha256":sha_bytes(araw),"roots_resolved":roots,"schemas":load_schemas(schema_root),"projection":json.loads(projection_path.read_text()),"matrix":matrix,"command_manifest":command_manifest,"authority_read_count":0}
...
def consume_once(context:Mapping[str,Any])->Path:
    ...
    except FileExistsError as exc:raise Repair03Error("ALREADY_CONSUMED") from exc
    return path
...
def load_objects(repo:Path,context:Mapping[str,Any])->tuple[dict[str,Any],list[dict[str,Any]],int]:
    ...
    if context["technical"]:
        ...
        if sha_file(bundle_path)!=release["technical_bundle_sha256"]:raise Repair03Error("TECHNICAL_BUNDLE_HASH")
        bundle,_=read_canonical(bundle_path)
        if bundle.get("identity_class")!="TECHNICAL_MIRROR_ONLY_NEVER_FORMAL_AUTHORITY":raise Repair03Error("TECHNICAL_BUNDLE_CLASS")
        return bundle["objects"],bundle["authority_entries"],0
    allow_path=_safe(repo,release["authority_allowlist_path"])
    if sha_file(allow_path)!=release["authority_allowlist_sha256"]:raise Repair03Error("ALLOWLIST_HASH")
    allow=json.loads(allow_path.read_text());objects={}
    for entry in allow["entries"]:
        path=_safe(repo,entry["path"])
        if sha_file(path)!=entry["sha256"]:raise Repair03Error("AUTHORITY_HASH")
        objects[entry["purpose"]]=pointer(json.loads(path.read_text()),entry["pointer"])
    return objects,allow["entries"],len(allow["entries"])
...
def build_staging(stage:Path,objects:Mapping[str,Any],entries:Sequence[Mapping[str,Any]],context:Mapping[str,Any])->dict[str,Any]:
    if stage.exists():raise Repair03Error("STAGE_EXISTS")
...
def write_terminal(context:Mapping[str,Any],success:bool,counts:Mapping[str,int],reason:str,pointer_sha:str|None=None)->Path:
    ...
    if sp.exists() or fp.exists():raise Repair03Error("SINGLE_TERMINAL")
...
def _temp(target:Path,run_id:str,index:int)->Path:
    ...
    return target.with_name(f".r03.{index:02d}.{suffix}.tmp")
...
def publish(stage:Path,run:Path,journal:Path,pointer_path:Path,context:Mapping[str,Any])->dict[str,Any]:
    ...
    if {p.relative_to(stage).as_posix() for p in stage.rglob("*") if p.is_file()}!=set(logical) or run.exists():raise Repair03Error("PUBLISH_INPUT")
    run.mkdir(parents=True);entries=[];fault=os.environ.get("GEN_ENC_B1_REPAIR03_FAULT") if context["technical"] else None
    state={"schema_version":"gen_enc_fast_b1_repair_03_publication_journal_v1","repair_id":REPAIR,"task_id":TASK_ID,"batch_id":BATCH_ID,"run_id":context["run_id"],"release_sha256":context["release_sha256"],"run_root_absolute":str(run.resolve()),"pointer_absolute":str(pointer_path.resolve()),"state":"PUBLISHING","entries":entries,"final_test_read":False}
    for index,rel in enumerate(logical):
        ...
        if fault==f"copy:{index}":raise Repair03Error("INJECT_COPY")
        with source.open("rb") as src,temp.open("xb") as dst:shutil.copyfileobj(src,dst);dst.flush();os.fsync(dst.fileno())
        if fault==f"fsync:{index}":raise Repair03Error("INJECT_FSYNC")
        os.replace(temp,target)
        if fault==f"kill:{index}":os._exit(86)
        if fault==f"replace:{index}":raise Repair03Error("INJECT_REPLACE")
        entry["phase"]="REPLACED";atomic_json(journal,state)
        if fault==f"journal:{index}":raise Repair03Error("INJECT_JOURNAL")
    pointer={"schema_version":"gen_enc_fast_b1_repair_03_commit_pointer_v1","record_kind":"ATOMIC_B1_COMMIT_POINTER","repair_id":REPAIR,"task_id":TASK_ID,"batch_id":BATCH_ID,"run_id":context["run_id"],"mode":MODE,"release_sha256":context["release_sha256"],"projection_contract_path":PROJECTION_PATH,"projection_contract_sha256":context["projection_contract_sha256"],"dependency_matrix_path":DEPENDENCY_PATH,"dependency_matrix_sha256":context["dependency_matrix_sha256"],"dependency_structure_sha256":context["dependency_structure_sha256"],"immutable_run_directory":context["roots"]["run"],"batch_index_sha256":sha_file(run/"batch_index.json"),"sha256sums_sha256":sha_file(run/"SHA256SUMS.txt"),"verification_sha256":sha_file(run/"independent_verification.json"),"artifact_count":31,"atomic":True,"final_endpoint":False,"claim_ceiling":CLAIM,"final_test_read":False};validate(pointer,context["schemas"],"commit_pointer")
    state={**state,"state":"COMMITTED","entries":[{**e,"phase":"REPLACED"} for e in entries]};atomic_json(journal,state)
    if fault=="pointer":raise Repair03Error("INJECT_POINTER")
    pointer_path.parent.mkdir(parents=True,exist_ok=True);atomic_json(pointer_path,pointer);return {"published":31,"pointer_sha256":sha_file(pointer_path)}


def rollback(journal:Path,run:Path,pointer:Path,context:Mapping[str,Any])->int:
    state,_=read_canonical(journal);removed=0;logical=exact_artifacts()
    for entry in reversed(state["entries"]):
        i=entry["index"];target=Path(entry["target_absolute"]);temp=Path(entry["temp_absolute"])
        if i>=31 or entry["logical_path"]!=logical[i] or target.resolve()!=(run/logical[i]).resolve() or temp!=_temp(target,state["run_id"],i) or entry["ownership_token"]!=_token(state["run_id"],i,target,temp):raise Repair03Error("RECOVERY_OWNERSHIP")
        for path in (temp,target):
            if not path.resolve().is_relative_to(run.resolve()):raise Repair03Error("RECOVERY_CONTAINMENT")
            if path.is_file():path.unlink();removed+=1
    if pointer.is_file():pointer.unlink();removed+=1
...
def package_results(context:Mapping[str,Any])->dict[str,Any]:
    ...
    if len(terminals)!=1:raise Repair03Error("TERMINAL_CARDINALITY")
    ...
    for key,value in {**_binding(context["release"]),"release_sha256":context["release_sha256"],"attestation_sha256":context["attestation_sha256"]}.items():
        if key in terminal and terminal[key]!=value:raise Repair03Error("PACKAGE_BINDING:"+key)
    if terminal["record_kind"]=="VERIFIED_SUCCESS":
        ...
        if terminal["commit_pointer_sha256"]!=sha_file(pointer_path):raise Repair03Error("PACKAGE_POINTER")
        ...
        for key,value in expected.items():
            if pointer.get(key)!=value:raise Repair03Error("PACKAGE_POINTER_BINDING:"+key)
        run=roots["run"]
        if {p.relative_to(run).as_posix() for p in run.rglob("*") if p.is_file()}!=set(exact_artifacts()):raise Repair03Error("PACKAGE_GRAPH")
        ...
        for record_name,record in (("INDEX",index),("REPORT",report)):
            for key,value in {k:v for k,v in expected.items() if k!="release_sha256"}.items():
                if record.get(key)!=value:raise Repair03Error("PACKAGE_"+record_name+"_BINDING:"+key)
        if (run/"SHA256SUMS.txt").read_bytes()!=_hash_lines(run):raise Repair03Error("PACKAGE_SHA256SUMS")
        if pointer["batch_index_sha256"]!=sha_file(run/"batch_index.json") or pointer["verification_sha256"]!=sha_file(run/"independent_verification.json") or pointer["sha256sums_sha256"]!=sha_file(run/"SHA256SUMS.txt"):raise Repair03Error("PACKAGE_RESEAL")
...
def _check_cli_paths(ns:argparse.Namespace,repo:Path,context:Mapping[str,Any],raw_argv:Sequence[str])->None:
    if ns.projection_contract.as_posix()!=PROJECTION_PATH or ns.dependency_matrix.as_posix()!=DEPENDENCY_PATH:raise Repair03Error("ARGV_PROJECTION_PATH")
    if ns.projection_contract.resolve()!=_safe(repo,context["projection_contract_path"]) or ns.dependency_matrix.resolve()!=_safe(repo,context["dependency_matrix_path"]):raise Repair03Error("ARGV_PROJECTION_RESOLUTION")
    if not context["technical"]:
        expected={"repo_root":Path("."),"release":Path("outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_03/control/RELEASE.json"),"guardian_attestation":Path("outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_03/control/GUARDIAN_ATTESTATION.json"),"schema_root":Path("schemas/gen_enc/b1_repair_03")}
        if any(getattr(ns,key)!=value for key,value in expected.items()):raise Repair03Error("ARGV_FORMAL_EXACT")
        row=next((x for x in context["command_manifest"]["entries"] if x["command"]==ns.command),None)
        if row is None or list(raw_argv)!=row["argv_after_python"][2:]:raise Repair03Error("ARGV_BYTE_EXACT")
    ...
    try:
        ...
        if ns.command=="preflight":
            for name in ("staging","run","journal","success","failure","pointer"):
                if roots[name].exists():raise Repair03Error("PREFLIGHT_ROOT_EXISTS:"+name)
            ...
            publication=publish(roots["staging"],roots["run"],roots["journal"]/"publication.json",roots["pointer"]/"B1.json",context);counts.update(members=20,artifacts=31,published=31);terminal=write_terminal(context,True,counts,"POINTER_LAST_PUBLISHED",publication["pointer_sha256"]);result={**publication,"terminal_sha256":sha_file(terminal),"status":"VERIFIED_SUCCESS"}
        elif ns.command=="recover":
            _observe_counts(roots,counts);journal=roots["journal"]/"publication.json";removed=rollback(journal,roots["run"],roots["pointer"]/"B1.json",context) if journal.is_file() else 0;counts["published"]=0;terminal=write_terminal(context,False,counts,"RECOVERY_FAIL_CLOSED");result={"status":"TECHNICAL_FAIL_CLOSED","removed":removed,"terminal_sha256":sha_file(terminal)}
        ...
        if context is not None and ns.command not in ("preflight","package-results"):
            ...
            if not sp.exists() and not fp.exists():
                if (roots["journal"]/"publication.json").is_file():rollback(roots["journal"]/"publication.json",roots["run"],roots["pointer"]/"B1.json",context)
