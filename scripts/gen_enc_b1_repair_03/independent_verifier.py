"""Standalone repair-03 verifier; imports neither driver nor any old implementation."""
...
from scripts.gen_enc_b1_repair_03 import cad_projection_verifier as cad
import jsonschema

from scripts.gen_enc_b1_repair_03.primitives import atomic_json, canonical, pointer, read_canonical, sha_bytes, sha_file
...
CLAIM="BATCH_LOCAL_IDENTITY_COMPLETE_CAD_STATIC_AND_TECHNICAL_VALIDITY_ONLY";PROJECTION_PATH="outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_03/cad_typed_projection_contract.json";DEPENDENCY_PATH="outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_03/cad_leaf_dependency_matrix.json"
...
MASK=(1<<64)-1;GAMMA=0x9E3779B97F4A7C15;RUN_DOMAIN=b"GEN-ENC-FAST-B1-PRE-RELEASE-REPAIR-03-RUN-ID-v1"
MODE="FORMAL_B1_EXACT_20_TYPED_CAD";COMMANDS=("preflight","generate-staging","verify","publish","recover","package-results")


class Verify03Error(RuntimeError):pass
...
def _bounds(spec:Mapping[str,Any],axes:Sequence[str],groups:Sequence[str])->dict[str,tuple[float,float]]:
    if tuple(spec.get("parameter_order",()))!=tuple(axes) or spec.get("member_count")!=20 or spec.get("dof")!=len(axes):raise Verify03Error("AXES")
    result={}
    if len(spec.get("parameters",[]))!=len(groups):raise Verify03Error("GROUPS")
    for position,name in enumerate(groups):
        item=spec["parameters"][position]
        if set(item)!={"group","axes","bounds"} or item["group"]!=name or len(item["bounds"])!=2:raise Verify03Error("GROUP")
        lo,hi=float(item["bounds"][0]),float(item["bounds"][1])
        if lo>=hi:raise Verify03Error("BOUND")
        for axis in item["axes"]:
            if axis in result:raise Verify03Error("DUP")
            result[axis]=(lo,hi)
    if list(result)!=list(axes):raise Verify03Error("GROUP_ORDER")
    return result
...
def expected_parameters(objects:Mapping[str,Any],family:str,ordinal:int)->dict[str,float]:
    ...
    if family==FAMILIES[2]:
        ...
        if spec.get("uniform_algorithm")!={"interval":"OPEN_0_1_BINARY64","word_to_uniform":"((word>>11)+0.5)/2^53","one_guard":"nextafter(1.0,0.0)"} or spec.get("splitmix64",{}).get("gamma_hex")!="9e3779b97f4a7c15":raise Verify03Error("RANDOM_SPEC")
    ...
    if spec.get("lhs_algorithm")!=["STRATA=20","SAMPLE=(PERMUTATION[row]+0.5)/20","JITTER=false","FISHER_YATES_SPLITMIX64=i19_TO_1"] or spec.get("lhs_master_seed")!=master:raise Verify03Error("PHYSICS_SPEC")
    out={}
...
def expected_evidence(family:str,p:Mapping[str,float],projected:Mapping[str,Any])->tuple[dict[str,Any],str]:
    ...
    for sector,weight in zip(SECTORS,weights):
        ...
        if abs(volume-target)>float(common["tolerance_m3"]):raise Verify03Error("RESIDUAL")
...
def verify_members(members:Sequence[Mapping[str,Any]],objects:Mapping[str,Any],entries:Sequence[Mapping[str,Any]],contract:Mapping[str,Any],binding:Mapping[str,Any])->dict[str,Any]:
    ...
    if projected["projection_contract_sha256"]!=binding["projection_contract_sha256"] or projected["dependency_matrix_sha256"]!=binding["dependency_structure_sha256"]:raise Verify03Error("PROJECTION_BINDING")
    expected_ids=[f"{PREFIX[f]}_{i:02d}" for f in FAMILIES for i in range(1,6)]
    if [m.get("member_id") for m in members]!=expected_ids:raise Verify03Error("MEMBER_SET_ORDER")
    for member in members:
        ...
        if member["parameters"]!=params or member["cad_static_evidence"]!=evidence or member["typed_projection_provenance"]!=provenance or member["static_status"]!=status:raise Verify03Error("DEEP_MEMBER:"+member["member_id"])
...
def pre_read_binding(record:Mapping[str,Any],expected:Mapping[str,Any])->None:
    for name in ("task_id","batch_id","run_id","projection_contract_path","projection_contract_sha256","dependency_matrix_path","dependency_matrix_sha256","dependency_structure_sha256"):
        if record.get(name)!=expected.get(name):raise Verify03Error("PRE_READ_BINDING:"+name)


def _safe(repo:Path,value:str)->Path:
    result=(repo/value).resolve()
    if result==repo or not result.is_relative_to(repo):raise Verify03Error("PATH_CONTAINMENT")
    return result
...
def _validate(value:Any,schemas:Mapping[str,Any],name:str)->None:
    ...
    except jsonschema.ValidationError as exc:raise Verify03Error("SCHEMA_"+name+":"+exc.message) from exc


def _roots(run_id:str,technical:bool)->dict[str,str]:
    base=(".technical_b1_repair_03/" if technical else "outputs/gen_enc/GEN_ENC_FAST_START/b1_repair_03_runs/")+run_id;result={x:f"{base}/{x}" for x in ("staging","run","journal","success","failure","side_records")};result["pointer"]=f".technical_b1_repair_03_commit/{run_id}" if technical else f"outputs/gen_enc/GEN_ENC_FAST_START/b1_repair_03_commit/{run_id}";return result
...
def gate(repo:Path,release_path:Path,attestation_path:Path,schema_root:Path,cli:argparse.Namespace|None=None,raw_argv:Sequence[str]=())->tuple[dict[str,Any],list[dict[str,Any]],dict[str,Any]]:
    ...
    if technical:
        if release.get("identity_class")!="TECHNICAL_MIRROR_RELEASE_NEVER_FORMAL" or not (repo/".technical_b1_repair_03_root").is_file():raise Verify03Error("TECH_ROOT")
    else:_validate(release,schemas,"release")
    if release.get("record_kind") not in ("RELEASE","TECHNICAL_MIRROR_RELEASE") or release.get("repair_id")!=REPAIR or release.get("task_id")!=TASK_ID or release.get("batch_id")!=BATCH_ID or release.get("mode")!=MODE or release.get("commands_exact")!=list(COMMANDS) or release["run_id"]!=derive_run_id(release):raise Verify03Error("RELEASE")
    if datetime.fromisoformat(release["expires_at"]).astimezone(timezone.utc)<=datetime.now(timezone.utc) or release["roots"]!=_roots(release["run_id"],technical):raise Verify03Error("EXPIRY_ROOT")
    ...
    if release["projection_contract_path"]!=PROJECTION_PATH or release["dependency_matrix_path"]!=DEPENDENCY_PATH or sha_file(projection_path)!=release["projection_contract_sha256"] or sha_file(matrix_path)!=release["dependency_matrix_sha256"]:raise Verify03Error("PROJECTION_FILE")
    matrix,_=read_canonical(matrix_path)
    if matrix.get("dependency_structure_sha256")!=release["dependency_structure_sha256"] or matrix.get("projection_contract_sha256")!=release["projection_contract_sha256"]:raise Verify03Error("STRUCTURE")
    for name in ("source","schema","command"):
        if sha_file(_safe(repo,release[f"{name}_manifest_path"]))!=release[f"{name}_manifest_sha256"]:raise Verify03Error("MANIFEST:"+name)
    command_manifest,_=read_canonical(_safe(repo,release["command_manifest_path"]))
    rows=command_manifest.get("entries",[])
    if command_manifest.get("mode")!=MODE or command_manifest.get("commands_exact")!=list(COMMANDS) or len(rows)!=6 or [row.get("command") for row in rows]!=list(COMMANDS):raise Verify03Error("COMMAND_MANIFEST")
    att,araw=read_canonical(attestation_path);expected=_file_binding(release)
    if att.get("record_kind") not in ("GUARDIAN_ONE_WAY_ATTESTATION","TECHNICAL_MIRROR_ONE_WAY_ATTESTATION") or att.get("release_sha256")!=sha_bytes(raw) or att.get("one_way") is not True:raise Verify03Error("ATTESTATION")
    for key,value in expected.items():
        if att.get(key)!=value:raise Verify03Error("ATTESTATION_BINDING:"+key)
    side=roots["side_records"]
    if (side/"REVOKED.json").exists():raise Verify03Error("REVOKED")
    for state in ("ISSUED","CONSUMED"):
        value,_=read_canonical(side/f"{state}.json")
        if value.get("state")!=state or value.get("revoked") is not False or value.get("release_sha256")!=sha_bytes(raw) or value.get("attestation_sha256")!=sha_bytes(araw):raise Verify03Error(state)
        for key,want in expected.items():
            if value.get(key)!=want:raise Verify03Error(state+"_BINDING:"+key)
    ...
    if technical:
        ...
        if sha_file(bundle_path)!=release["technical_bundle_sha256"]:raise Verify03Error("BUNDLE_HASH")
        bundle,_=read_canonical(bundle_path)
        if bundle.get("identity_class")!="TECHNICAL_MIRROR_ONLY_NEVER_FORMAL_AUTHORITY":raise Verify03Error("BUNDLE_CLASS")
        objects,entries,reads=bundle["objects"],bundle["authority_entries"],0
    else:
        allow_path=_safe(repo,release["authority_allowlist_path"])
        if sha_file(allow_path)!=release["authority_allowlist_sha256"]:raise Verify03Error("ALLOWLIST")
        allow=json.loads(allow_path.read_text());objects={}
        for entry in allow["entries"]:
            path=_safe(repo,entry["path"])
            if sha_file(path)!=entry["sha256"]:raise Verify03Error("AUTHORITY_HASH")
            objects[entry["purpose"]]=pointer(json.loads(path.read_text()),entry["pointer"])
        entries,reads=allow["entries"],len(allow["entries"])
    return objects,entries,{**expected,"technical":technical,"release":release,"release_sha256":sha_bytes(raw),"attestation_sha256":sha_bytes(araw),"roots_resolved":roots,"schemas":schemas,"projection":json.loads(projection_path.read_text()),"matrix":matrix,"command_manifest":command_manifest,"authority_read_count":reads}
...
def verify_stage(stage:Path,objects:Mapping[str,Any],entries:Sequence[Mapping[str,Any]],context:Mapping[str,Any])->dict[str,Any]:
    ...
    if {p.relative_to(stage).as_posix() for p in stage.rglob("*") if p.is_file()}!=pending:raise Verify03Error("PENDING_GRAPH")
    ...
    for family in FAMILIES:
        ...
        if manifest["member_entries"]!=expected or manifest["slot_ordinals"]!=[1,2,3,4,5]:raise Verify03Error("MANIFEST_GRAPH")
    ...
    if (stage/"SHA256SUMS.txt").read_text(encoding="ascii")!=expected_sums:raise Verify03Error("PENDING_SHA256SUMS")
    for key,want in (("mode",MODE),("projection_contract_path",PROJECTION_PATH),("projection_contract_sha256",context["projection_contract_sha256"]),("dependency_matrix_path",DEPENDENCY_PATH),("dependency_matrix_sha256",context["dependency_matrix_sha256"]),("dependency_structure_sha256",context["dependency_structure_sha256"]),("member_paths",[f"members/{PREFIX[f]}_{i:02d}.json" for f in FAMILIES for i in range(1,6)])):
        if index.get(key)!=want:raise Verify03Error("INDEX:"+key)
    if audit.get("member_count")!=20 or len(audit.get("member_paths",[]))!=20:raise Verify03Error("AUDIT")
    report={"schema_version":"gen_enc_fast_b1_repair_03_independent_verification_v1","record_kind":"INDEPENDENT_VERIFICATION","repair_id":REPAIR,"task_id":TASK_ID,"batch_id":BATCH_ID,"run_id":context["run_id"],"status":"PASS","verified_member_count":20,"verified_manifest_count":4,"authority_read_count":context["authority_read_count"],"artifact_count":31,"checks":["TYPED_CAD_349_LEAVES","20_MEMBER_DEEP_RECOMPUTE","31_GRAPH_ZERO_UNLISTED"],"resealed":True,"projection_contract_path":PROJECTION_PATH,"projection_contract_sha256":context["projection_contract_sha256"],"dependency_matrix_path":DEPENDENCY_PATH,"dependency_matrix_sha256":context["dependency_matrix_sha256"],"dependency_structure_sha256":context["dependency_structure_sha256"],"mode":MODE,"claim_ceiling":CLAIM,"final_test_read":False};atomic_json(stage/"independent_verification.json",report);index["verification_complete"]=True;atomic_json(stage/"batch_index.json",index)
    verifier_terminal={"schema_version":"gen_enc_fast_b1_repair_03_verifier_terminal_v1","record_kind":"VERIFIED_STAGING_NON_PUBLICATION","repair_id":REPAIR,"task_id":TASK_ID,"batch_id":BATCH_ID,"run_id":context["run_id"],"status":"VERIFIED_RESEALED_AWAITING_PUBLICATION","authority_read_count":context["authority_read_count"],"verified_member_count":20,"artifact_count":31,"publication_started":False,"success_terminal_written":False,"claim_ceiling":CLAIM,"final_test_read":False};atomic_json(stage/"verifier_terminal.json",verifier_terminal)
    publication={"schema_version":"gen_enc_fast_b1_repair_03_publication_terminal_v1","record_kind":"PUBLICATION_INTENT_NON_SUCCESS","repair_id":REPAIR,"task_id":TASK_ID,"batch_id":BATCH_ID,"run_id":context["run_id"],"status":"AWAITING_MULTI_TARGET_ATOMIC_PUBLICATION","target_count":31,"published_count":0,"commit_pointer_written":False,"success_terminal_written":False,"claim_ceiling":CLAIM,"final_test_read":False};atomic_json(stage/"publication_terminal.json",publication)
    ...
    if len([p for p in stage.rglob("*") if p.is_file()])!=31:raise Verify03Error("VERIFIED_CARDINALITY")
...
def write_failure(context:Mapping[str,Any],reason:str)->Path:
    ...
    if success.exists():raise Verify03Error("SUCCESS_FORBIDS_FAILURE")
...
def _check_cli_paths(ns:argparse.Namespace,repo:Path,context:Mapping[str,Any],raw_argv:Sequence[str])->None:
    if ns.projection_contract.as_posix()!=PROJECTION_PATH or ns.dependency_matrix.as_posix()!=DEPENDENCY_PATH:raise Verify03Error("ARGV_PROJECTION_PATH")
    if ns.projection_contract.resolve()!=_safe(repo,context["projection_contract_path"]) or ns.dependency_matrix.resolve()!=_safe(repo,context["dependency_matrix_path"]):raise Verify03Error("ARGV_PROJECTION_RESOLUTION")
    if not context["technical"]:
        ...
        if any(getattr(ns,key)!=value for key,value in expected.items()):raise Verify03Error("ARGV_FORMAL_EXACT")
        row=next((x for x in context["command_manifest"]["entries"] if x["command"]=="verify"),None)
        if row is None or list(raw_argv)!=row["argv_after_python"][2:]:raise Verify03Error("ARGV_BYTE_EXACT")


def main(argv:Sequence[str]|None=None)->int:
    raw_argv=list(sys.argv[1:] if argv is None else argv);ns=parser().parse_args(raw_argv);context=None
    try:
        repo=ns.repo_root.resolve();objects,entries,context=gate(repo,ns.release.resolve(),ns.guardian_attestation.resolve(),ns.schema_root.resolve(),ns,raw_argv);result=verify_stage(context["roots_resolved"]["staging"],objects,entries,context);print(json.dumps(result,sort_keys=True));return 0
    except BaseException as exc:
        if context is not None:write_failure(context,type(exc).__name__+":"+str(exc))
