"""Build strict repair-04 lifecycle and typed-CAD schemas."""
...
REPO=Path(__file__).resolve().parents[2];OLD=REPO/"schemas/gen_enc/b1_repair_02";OUT=REPO/"schemas/gen_enc/b1_repair_04"
...
def main()->None:
    ...
    member={"$schema":"https://json-schema.org/draft/2020-12/schema",**infer(members)};member["$id"]="gen_enc_fast_b1_repair_04_member_identity_v1";member["properties"]["schema_version"]={"const":"gen_enc_fast_b1_repair_04_member_identity_v1"};member["properties"]["record_kind"]={"const":"BATCH_LOCAL_MEMBER_IDENTITY_TYPED_CAD_DERIVED"};member["properties"]["task_id"]={"const":driver.TASK_ID};member["properties"]["repair_id"]={"const":driver.REPAIR};member["properties"]["batch_id"]={"const":"B1"};member["properties"]["final_test_read"]={"const":False}
    schemas={"member_identity":member,"typed_projection":{"$schema":"https://json-schema.org/draft/2020-12/schema","$id":"gen_enc_fast_b1_repair_04_typed_projection_v1",**infer([projected])}}
    for path in OLD.glob("*.schema.json"):
        if path.stem=="member_identity.schema":continue
        name=path.name.removesuffix(".schema.json");value=json.loads(path.read_text());value=json.loads(json.dumps(value).replace("repair_02","repair_04").replace("PRE-RELEASE-REPAIR-02","PRE-RELEASE-REPAIR-04"))
    ...
    for key,path in (("source_manifest_path","outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_04/source_manifest.json"),("schema_manifest_path","outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_04/schema_manifest.json"),("command_manifest_path","outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_04/command_manifest.json")):
        side["properties"][key]={"const":path};side["required"].append(key)
    receipt=copy.deepcopy(schemas["side_record"]);receipt["$id"]="gen_enc_fast_b1_repair_04_authority_read_receipt_v1"
    for key in ("state","one_use","revoked"):receipt["properties"].pop(key);receipt["required"].remove(key)
    receipt["properties"].update({"schema_version":{"const":"gen_enc_fast_b1_repair_04_authority_read_receipt_v1"},"record_kind":{"const":"CUMULATIVE_AUTHORITY_READ_RECEIPT"},"ordinal":{"type":"integer","minimum":1,"maximum":100},"phase":{"enum":["DRIVER_GENERATION","INDEPENDENT_VERIFICATION"]},"purpose":{"type":"string","minLength":1},"path_label":{"type":"string","minLength":1},"expected_sha256":{"type":"string","pattern":"^[0-9a-f]{64}$"}})
