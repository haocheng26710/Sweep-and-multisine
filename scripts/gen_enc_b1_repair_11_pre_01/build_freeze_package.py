...
import jsonschema
from .build_schemas import ALLOWED, FORBIDDEN

TASK="01a049a7-15ca-79e1-92a2-d3822ba8609d"
PACKAGE=Path("outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_11_shape_only_preexecution_corr01")
...
ADVISOR_REVIEW=Path("outputs/governance/RESEARCH_ADVANCEMENT_SCOPE_INTEGRITY_ADVISOR/reviews/GEN_ENC_FAST_B1_REPAIR_11_SHAPE_ONLY_IMPLEMENTATION_FREEZE_PREEXECUTION_REVIEW.json")
ADVISOR_REVIEW_SHA="cc510911b2dbd1a902a018bfce8f3249518aa4140827163e6e1a39e1a208189f"
...
def commands()->dict[str,Any]:
    common=["--repo-root",".","--release","<RELEASE_EXACT_PATH>","--guardian-attestation","<ATTESTATION_EXACT_PATH>","--dispatch","<DISPATCH_EXACT_PATH>","--schema-root","schemas/gen_enc/b1_repair_11_pre_01"]
    entries=[]
    for command in ("shape-preflight","extract-shape","verify-shape","package-shape"):
        module="scripts.gen_enc_b1_repair_11_pre_01.independent_verifier" if command=="verify-shape" else "scripts.gen_enc_b1_repair_11_pre_01.formal_driver"
...
def build(test_history:dict[str,Any]|None=None)->None:
    ...
    if sha_file(ADVISOR_REVIEW)!=ADVISOR_REVIEW_SHA:raise RuntimeError("ADVISOR_REVIEW_HASH")
    ...
    output={"schema_version":"gen_enc_b1_r11_pre_01_output_manifest_v1","record_kind":"SHAPE_ONLY_OUTPUT_ALLOWLIST","task_id":TASK,"allowed_run_relative_paths":ALLOWED,"scientific_artifacts_allowed":False,"member_artifacts_allowed":False,"publication_allowed":False,"final_test_read":False};validate(output,"output_manifest");write(PACKAGE/"output_manifest.json",output)
    negative={"schema_version":"gen_enc_b1_r11_pre_01_negative_capability_manifest_v1","record_kind":"R11_NEGATIVE_CAPABILITY_FREEZE","task_id":TASK,"forbidden":FORBIDDEN,"ownership_cells_classification":"FIXTURE_ONLY_FUTURE_OUTPUT","ownership_rule_classification":"FIXTURE_ONLY_FUTURE_OUTPUT","pointer_substitution_allowed":False,"formal_authority_read_count":0,"final_test_read":False};validate(negative,"negative_capability_manifest");write(PACKAGE/"negative_capability_manifest.json",negative)
    source_paths=[Path("scripts/gen_enc_b1_repair_11_pre_01")/name for name in ("__init__.py","driver_core.py","verifier_core.py","driver_runtime.py","verifier_runtime.py","formal_driver.py","independent_verifier.py","technical_driver.py","technical_verifier.py","build_schemas.py","build_freeze_package.py")]+[Path("tests/helpers/gen_enc_b1_repair_11_pre_01_fixture.py"),Path("tests/test_gen_enc_b1_repair_11_pre_01.py"),Path("tests/test_gen_enc_b1_repair_11_pre_01_blockers.py")]
    ...
    draft={"schema_version":"gen_enc_b1_r11_pre_01_draft_v1","record_kind":"DRAFT","task_id":TASK,"repair_id":"PRE-RELEASE-REPAIR-11-PREEXECUTION-CORR01","attempt":0,"authoritative":False,"run_id":None,"permissions":{name:False for name in ("authority_shape_read","shape_controls","shape_run","identity","static","publication","phase_b")},"final_test_read":False};validate(draft,"draft");write(PACKAGE/"DRAFT.json",draft)
    provenance={"schema_version":"gen_enc_b1_r11_pre_01_authority_provenance_v1","record_kind":"IMPLEMENTATION_FREEZE_AUTHORITY_PROVENANCE","task_id":TASK,"guardian_contract":{"path":CONTRACT.as_posix(),"sha256":CONTRACT_SHA},"advisor_b_supervision":{"path":SUPERVISION.as_posix(),"sha256":SUPERVISION_SHA},"advisor_b_blocker_review":{"path":ADVISOR_REVIEW.as_posix(),"sha256":ADVISOR_REVIEW_SHA},"prior_r11_package":{"path":"outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_11_shape_only_freeze","sha256sums_sha256":"cfa59491c9ea28b567c4a5d98d4b28dbc624c24809ac30ca75236c12a7b9a659"},"r10_projection_metadata":{"path":R10_PROJECTION.as_posix(),"sha256":sha_file(R10_PROJECTION)},"formal_authority_files_opened":0,"formal_controls_created":0,"formal_run_id":None,"final_test_read":False};write(PACKAGE/"authority_provenance.json",provenance)
