...
from scripts.gen_enc_b1_repair_06.primitives import pointer, sha_value
...
def project(objects:Mapping[str,Any],entries_input:Sequence[Mapping[str,Any]],contract:Mapping[str,Any])->dict[str,Any]:
    ...
    return {"schema_version":"gen_enc_fast_b1_repair_06_typed_cad_projection_v1","values":values,"semantic_leaf_count":len(matrix),"dependency_matrix":matrix,"dependency_matrix_sha256":sha_value(structural),"unused_leaf_allowlist":unused,"unused_leaf_count":len(unused),"projection_contract_sha256":sha_value(contract)}
