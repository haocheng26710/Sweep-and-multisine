"""Standalone Repair-08 two-phase driver with permanent Phase-A failure semantics."""
...
from scripts.gen_enc_b1_repair_08.primitives import atomic_json, canonical, pointer, read_canonical, sha_bytes, sha_file, sha_value

TASK_ID="01a049a7-15ca-79e1-92a2-d3822ba8609d";BATCH_ID="B1";REPAIR="PRE-RELEASE-REPAIR-08"
...
RUN_DOMAIN=b"GEN-ENC-FAST-B1-PRE-RELEASE-REPAIR-08-RUN-ID-v1"
PROJECTION_PATH="outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_08/cad_typed_projection_contract.json"
DEPENDENCY_PATH="outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_08/cad_leaf_dependency_matrix.json"
...
class Repair08Error(RuntimeError):pass
...
def _groups(spec:Mapping[str,Any],axes:Sequence[str],names:Sequence[str])->dict[str,tuple[float,float]]:
    if tuple(spec.get("parameter_order",()))!=tuple(axes) or spec.get("member_count")!=20 or spec.get("dof")!=len(axes):raise Repair08Error("SPEC_AXES")
    result={}
    for item,name in zip(spec.get("parameters",[]),names):
        if set(item)!={"group","axes","bounds"} or item["group"]!=name or len(item["bounds"])!=2:raise Repair08Error("SPEC_GROUP")
        lo,hi=map(float,item["bounds"])
        if not lo<hi:raise Repair08Error("SPEC_BOUND")
        for axis in item["axes"]:
            if axis in result:raise Repair08Error("DUP_AXIS")
            result[axis]=(lo,hi)
    if tuple(result)!=tuple(axes):raise Repair08Error("GROUP_AXIS_ORDER")
    return result
...
def parameters(objects:Mapping[str,Any],family:str,ordinal:int)->dict[str,float]:
    ...
    if family==FAMILIES[2]:
        ...
        if spec.get("uniform_algorithm")!={"interval":"OPEN_0_1_BINARY64","word_to_uniform":"((word>>11)+0.5)/2^53","one_guard":"nextafter(1.0,0.0)"} or spec.get("splitmix64",{}).get("gamma_hex")!="9e3779b97f4a7c15":raise Repair08Error("RANDOM_ALGORITHM")
    ...
    if spec.get("lhs_algorithm")!=["STRATA=20","SAMPLE=(PERMUTATION[row]+0.5)/20","JITTER=false","FISHER_YATES_SPLITMIX64=i19_TO_1"] or spec.get("lhs_master_seed")!=master:raise Repair08Error("PHYSICS_ALGORITHM")
    result={}
...
def derive_cad_evidence(family:str,params:Mapping[str,float],projection:Mapping[str,Any])->tuple[dict[str,Any],str]:
    ...
    for sector,weight in zip(SECTORS,weights):
        ...
        if abs(measured-target)>float(common["tolerance_m3"]):raise Repair08Error("ROOT_TOLERANCE")
...
def pre_read_binding(record:Mapping[str,Any],expected:Mapping[str,Any])->None:
    for key in ("task_id","batch_id","run_id","projection_contract_path","projection_contract_sha256","dependency_matrix_path","dependency_matrix_sha256","dependency_structure_sha256"):
        if record.get(key)!=expected.get(key):raise Repair08Error("PRE_READ_BINDING:"+key)


def _safe(repo:Path,value:str)->Path:
    path=(repo/value).resolve()
    if path==repo or not path.is_relative_to(repo):raise Repair08Error("PATH_CONTAINMENT")
    return path
...
def validate(value:Any,schemas:Mapping[str,Any],name:str)->None:
    ...
    except jsonschema.ValidationError as exc:raise Repair08Error("SCHEMA_"+name+":"+exc.message) from exc


def exact_roots(run_id:str,technical:bool=False)->dict[str,str]:
    base=(".technical_b1_repair_08/" if technical else "outputs/gen_enc/GEN_ENC_FAST_START/b1_repair_08_runs/")+run_id
    roots={x:f"{base}/{x}" for x in ("staging","run","journal","success","failure","side_records")};roots["pointer"]=(f".technical_b1_repair_08_commit/{run_id}" if technical else f"outputs/gen_enc/GEN_ENC_FAST_START/b1_repair_08_commit/{run_id}");return roots
...
def _validate_signing_authority_bytes(repo:Path,release:Mapping[str,Any],technical:bool)->None:
    ...
    for label,path_key,sha_key in (("FINAL_REVIEW","advisor_b_final_review_path","advisor_b_final_review_sha256"),("LEDGER_SNAPSHOT","advisor_b_ledger_snapshot_path","advisor_b_ledger_snapshot_sha256")):
        ...
        if not isinstance(path_value,str) or not path_value.endswith(".json") or not isinstance(expected_sha,str) or len(expected_sha)!=64 or any(c not in "0123456789abcdef" for c in expected_sha):raise Repair08Error("ADVISOR_DYNAMIC_BINDING:"+label)
        ...
        if not path_value.startswith(required_root):raise Repair08Error("ADVISOR_DYNAMIC_ROOT:"+label)
        value,raw=read_canonical(_safe(repo,path_value))
        if sha_bytes(raw)!=expected_sha:raise Repair08Error("ADVISOR_DYNAMIC_HASH:"+label)
        if technical and (value.get("identity_class")!="TASK_OWNED_SYNTHETIC_IMMUTABLE_NEVER_FORMAL" or value.get("subject")!=label or value.get("task_id")!=TASK_ID or value.get("final_test_read") is not False):raise Repair08Error("ADVISOR_TECHNICAL_SYNTHETIC_CLASS:"+label)
...
def validate_control(repo:Path,release_path:Path,attestation_path:Path,schema_root:Path,command:str)->dict[str,Any]:
    ...
    if technical:
        if release.get("identity_class")!="TECHNICAL_MIRROR_RELEASE_NEVER_FORMAL" or not (repo/".technical_b1_repair_08_root").is_file():raise Repair08Error("TECHNICAL_ROOT_MARKER")
    else:validate(release,load_schemas(schema_root),"release")
    if release.get("record_kind") not in ("PHASE_A_RELEASE","TECHNICAL_MIRROR_RELEASE") or release.get("repair_id")!=REPAIR or release.get("task_id")!=TASK_ID or release.get("batch_id")!=BATCH_ID or release.get("mode")!=MODE or release.get("commands_exact")!=list(PHASE_A_COMMANDS) or command not in COMMANDS:raise Repair08Error("PHASE_A_RELEASE_CONTRACT")
    if release.get("phase_a_commands_exact")!=list(PHASE_A_COMMANDS) or release.get("phase_a_permissions")!=PHASE_A_PERMISSIONS or release.get("phase_a_dispatch_path")!="control/PHASE_A_DISPATCH_AUTHORIZATION.json" or release.get("guardian_phase_b_authorization_path")!=PHASE_B_AUTH_PATH or release.get("guardian_phase_b_attestation_path")!=PHASE_B_ATTESTATION_PATH or release.get("phase_b_dispatch_path")!=PHASE_B_DISPATCH_PATH:raise Repair08Error("PHASE_SPLIT_CONTRACT")
    if release.get("guardian_two_phase_contract_path")!=GUARDIAN_TWO_PHASE_CONTRACT_PATH or release.get("guardian_two_phase_contract_sha256")!=GUARDIAN_TWO_PHASE_CONTRACT_SHA256 or release.get("guardian_two_phase_review_path")!=GUARDIAN_TWO_PHASE_REVIEW_PATH or release.get("guardian_two_phase_review_sha256")!=GUARDIAN_TWO_PHASE_REVIEW_SHA256:raise Repair08Error("GUARDIAN_TWO_PHASE_AUTHORITY_BINDING")
    if release.get("authority_allowlist_path")!=AUTHORITY_ALLOWLIST_PATH:raise Repair08Error("AUTHORITY_ALLOWLIST_PATH")
    if sha_file(_safe(repo,GUARDIAN_TWO_PHASE_CONTRACT_PATH))!=GUARDIAN_TWO_PHASE_CONTRACT_SHA256 or sha_file(_safe(repo,GUARDIAN_TWO_PHASE_REVIEW_PATH))!=GUARDIAN_TWO_PHASE_REVIEW_SHA256:raise Repair08Error("GUARDIAN_TWO_PHASE_AUTHORITY_BYTES")
    _validate_signing_authority_bytes(repo,release,technical)
    if release["run_id"]!=derive_run_id(release):raise Repair08Error("RUN_ID")
    if datetime.fromisoformat(release["expires_at"]).astimezone(timezone.utc)<=datetime.now(timezone.utc):raise Repair08Error("EXPIRED")
    if release["roots"]!=exact_roots(release["run_id"],technical):raise Repair08Error("ROOT_TEMPLATE")
    roots={k:_safe(repo,v) for k,v in release["roots"].items()};vals=list(roots.values())
    if len({str(x).casefold() for x in vals})!=len(vals) or any(a.is_relative_to(b) or b.is_relative_to(a) for i,a in enumerate(vals) for b in vals[i+1:]):raise Repair08Error("ROOT_EXCLUSIVITY")
    ...
    if release["projection_contract_path"]!=PROJECTION_PATH or release["dependency_matrix_path"]!=DEPENDENCY_PATH:raise Repair08Error("PROJECTION_PATH")
    projection_path=_safe(repo,release["projection_contract_path"]);matrix_path=_safe(repo,release["dependency_matrix_path"])
    if sha_file(projection_path)!=release["projection_contract_sha256"] or sha_file(matrix_path)!=release["dependency_matrix_sha256"]:raise Repair08Error("PROJECTION_FILE_HASH")
    matrix,_=read_canonical(matrix_path)
    if matrix.get("dependency_structure_sha256")!=release["dependency_structure_sha256"] or matrix.get("projection_contract_sha256")!=release["projection_contract_sha256"]:raise Repair08Error("DEPENDENCY_STRUCTURE")
    for name in ("source","schema","command"):
        if sha_file(_safe(repo,release[f"{name}_manifest_path"]))!=release[f"{name}_manifest_sha256"]:raise Repair08Error("MANIFEST_HASH:"+name)
    command_manifest,_=read_canonical(_safe(repo,release["command_manifest_path"]))
    rows=command_manifest.get("entries",[])
    if command_manifest.get("mode")!=MODE or command_manifest.get("commands_exact")!=list(COMMANDS) or len(rows)!=6 or [row.get("command") for row in rows]!=list(COMMANDS):raise Repair08Error("COMMAND_MANIFEST")
    ...
    if attestation.get("record_kind") not in ("GUARDIAN_ONE_WAY_ATTESTATION","TECHNICAL_MIRROR_ONE_WAY_ATTESTATION") or attestation.get("release_sha256")!=sha_bytes(raw) or attestation.get("one_way") is not True:raise Repair08Error("ATTESTATION")
    expected=_binding(release)
    for key,value in expected.items():
        if attestation.get(key)!=value:raise Repair08Error("ATTESTATION_BINDING:"+key)
    ...
    if failure.exists():
        ...
        for key,want in {"task_id":TASK_ID,"batch_id":BATCH_ID,"run_id":release["run_id"],"mode":MODE,"release_sha256":sha_bytes(raw),"attestation_sha256":sha_bytes(araw),"projection_contract_sha256":release["projection_contract_sha256"],"dependency_matrix_sha256":release["dependency_matrix_sha256"],"dependency_structure_sha256":release["dependency_structure_sha256"]}.items():
            if failed.get(key)!=want:raise Repair08Error("PHASE_A_FAILURE_BINDING:"+key)
        if failed.get("terminal_phase")=="PHASE_A":raise Repair08Error("PHASE_A_FAILURE_PERMANENT_NEW_RUN_REQUIRED")
    if not issued_path.is_file():raise Repair08Error("ISSUED_MISSING")
    ...
    if command=="preflight":
        ...
        if consumed_value.get("state")!="PHASE_A_CONSUMED" or consumed_value.get("phase")!="A" or consumed_value.get("dispatch_id")!=release["phase_a_dispatch_id"]:raise Repair08Error("PHASE_A_CONSUMED")
        for key,value in {**expected,"release_sha256":sha_bytes(raw),"attestation_sha256":sha_bytes(araw)}.items():
            if consumed_value.get(key)!=value:raise Repair08Error("CONSUMED_BINDING:"+key)
...
def consume_once(context:Mapping[str,Any])->Path:
    roots=context["roots_resolved"];issued,_=read_canonical(_safe(Path(context["repo_root"]),context["release"]["phase_a_dispatch_path"]));value={**issued,"schema_version":"gen_enc_fast_b1_repair_08_technical_phase_a_dispatch_consumed_v1" if context["technical"] else "gen_enc_fast_b1_repair_08_phase_a_dispatch_consumed_v1","state":"PHASE_A_CONSUMED"};path=roots["side_records"]/"PHASE_A_CONSUMED.json";path.parent.mkdir(parents=True,exist_ok=True)
    ...
    except FileExistsError as exc:raise Repair08Error("ALREADY_CONSUMED") from exc
    return path


def _external_record(repo:Path,path_arg:Path|None,expected_sha:str|None,exact_path:str,error:str)->tuple[dict[str,Any],str]:
    if path_arg is None or expected_sha is None or path_arg.as_posix()!=exact_path or len(expected_sha)!=64 or any(c not in "0123456789abcdef" for c in expected_sha):raise Repair08Error(error+":EXTERNAL_EXPECTED_REQUIRED")
    path=_safe(repo,exact_path);raw=path.read_bytes()
    if sha_bytes(raw)!=expected_sha:raise Repair08Error(error+":EXPECTED_SHA256")
    value,canonical_raw=read_canonical(path)
    if canonical_raw!=raw:raise Repair08Error(error+":CANONICAL")
...
def validate_phase_b(repo:Path,ns:argparse.Namespace,context:Mapping[str,Any],consume:bool)->dict[str,Any]:
    ...
    if failure.exists():
        ...
        if failed.get("terminal_phase")=="PHASE_A":raise Repair08Error("PHASE_A_FAILURE_FORBIDS_PHASE_B")
    ...
    if context["technical"]:
        if auth.get("record_kind")!="TECHNICAL_MIRROR_GUARDIAN_PHASE_B_AUTHORIZATION" or att.get("record_kind")!="TECHNICAL_MIRROR_GUARDIAN_PHASE_B_ONE_WAY_ATTESTATION" or dispatch.get("record_kind")!="TECHNICAL_PHASE_DISPATCH" or auth.get("authoritative") is not False or att.get("authoritative") is not False:raise Repair08Error("PHASE_B_TECHNICAL_CLASS")
    elif auth.get("record_kind")!="GUARDIAN_PHASE_B_PUBLISH_AUTHORIZATION" or att.get("record_kind")!="GUARDIAN_PHASE_B_ONE_WAY_ATTESTATION" or dispatch.get("record_kind")!="PHASE_B_TASK_BOUND_DISPATCH" or auth.get("authoritative") is not True or att.get("authoritative") is not True:raise Repair08Error("PHASE_B_FORMAL_CLASS")
    for name,record in (("AUTH",auth),("ATTESTATION",att),("DISPATCH",dispatch)):
        if record.get("one_use") is not True or record.get("revoked") is not False or datetime.fromisoformat(record["expires_at"]).astimezone(timezone.utc)<=datetime.now(timezone.utc):raise Repair08Error("PHASE_B_"+name+"_LIFETIME")
    handoff_payload=dict(handoff);handoff_digest=handoff_payload.pop("handoff_digest")
    if handoff_digest!=sha_value({"domain":"GEN-ENC-FAST-TWO-PHASE-HANDOFF-v1","payload":handoff_payload}):raise Repair08Error("PHASE_A_HANDOFF_DIGEST")
    ...
    for key,want in expected_handoff.items():
        if handoff.get(key)!=want:raise Repair08Error("PHASE_A_HANDOFF_BINDING:"+key)
    ...
    for name,record in (("AUTH",auth),("ATTESTATION",att),("DISPATCH",dispatch)):
        for key,want in common.items():
            if record.get(key)!=want:raise Repair08Error("PHASE_B_"+name+"_BINDING:"+key)
    if att.get("authorization_sha256")!=auth_sha or att.get("authorization_id")!=auth.get("authorization_id") or att.get("one_way") is not True:raise Repair08Error("PHASE_B_ATTESTATION_LINK")
    if dispatch.get("authorization_sha256")!=auth_sha or dispatch.get("phase_b_attestation_sha256")!=phase_b_att_sha or dispatch.get("authorization_id")!=auth.get("authorization_id") or dispatch.get("phase_a_dispatch_id")!=context["release"]["phase_a_dispatch_id"] or dispatch.get("dispatch_id")==context["release"]["phase_a_dispatch_id"]:raise Repair08Error("PHASE_B_DISPATCH_LINK")
    if handoff.get("formal_publication_count")!=0 or handoff.get("commit_pointer_count")!=0 or handoff.get("success_package_count")!=0 or handoff.get("final_test_read") is not False:raise Repair08Error("PHASE_A_HANDOFF_ZERO_STATE")
    consumed_path=context["roots_resolved"]["side_records"]/"PHASE_B_CONSUMED.json"
    if consume and consumed_path.exists():raise Repair08Error("PHASE_B_ALREADY_CONSUMED")
    if consume:
        for name in ("run","journal","pointer","success","failure"):
            if context["roots_resolved"][name].exists():raise Repair08Error("PHASE_B_PUBLISH_DIRTY_PRECOPY:"+name)
    ...
    if handoff.get("artifact_graph_entries")!=graph_entries or handoff.get("artifact_graph_digest")!=sha_value({"domain":"GEN-ENC-FAST-PHASE-A-31-GRAPH-v1","entries":graph_entries}):raise Repair08Error("PHASE_A_HANDOFF_GRAPH_CHANGED")
    if consume:
        if dispatch.get("state")!="PHASE_B_ISSUED" or consumed_path.exists():raise Repair08Error("PHASE_B_ALREADY_CONSUMED")
        ...
        except FileExistsError as exc:raise Repair08Error("PHASE_B_ALREADY_CONSUMED") from exc
    else:
        consumed,_=read_canonical(consumed_path);validate(consumed,context["schemas"],"phase_b_dispatch_consumed")
        if consumed.get("state")!="PHASE_B_CONSUMED" or consumed.get("phase")!="B" or consumed.get("dispatch_expected_sha256")!=dispatch_sha or consumed.get("dispatch_id")!=dispatch.get("dispatch_id"):raise Repair08Error("PHASE_B_CONSUMED_BINDING")
        for key,want in common.items():
            if consumed.get(key)!=want:raise Repair08Error("PHASE_B_CONSUMED_BINDING:"+key)
...
def authority_read_count(context:Mapping[str,Any])->int:
    ...
    if [p.name for p in paths]!=[f"{i:04d}.json" for i in range(1,len(paths)+1)]:raise Repair08Error("READ_RECEIPT_SEQUENCE")
    for ordinal,path in enumerate(paths,1):
        value,_=read_canonical(path);validate(value,context["schemas"],"authority_read_receipt")
        if value.get("ordinal")!=ordinal:raise Repair08Error("READ_RECEIPT_ORDINAL")
        for key,want in expected.items():
            if value.get(key)!=want:raise Repair08Error("READ_RECEIPT_BINDING:"+key)
    return len(paths)
...
def record_authority_read(context:Mapping[str,Any],phase:str,purpose:str,path_label:str,expected_sha256:str)->int:
    ...
    value={"schema_version":"gen_enc_fast_b1_repair_08_authority_read_receipt_v1","record_kind":"CUMULATIVE_AUTHORITY_READ_RECEIPT","repair_id":REPAIR,**_receipt_binding(context),"ordinal":ordinal,"phase":phase,"purpose":purpose,"path_label":path_label,"expected_sha256":expected_sha256,"final_test_read":False};validate(value,context["schemas"],"authority_read_receipt")
...
def load_objects(repo:Path,context:Mapping[str,Any])->tuple[dict[str,Any],list[dict[str,Any]],int]:
    ...
    if context["technical"]:
        ...
        if sha_file(bundle_path)!=release["technical_bundle_sha256"]:raise Repair08Error("TECHNICAL_BUNDLE_HASH")
        bundle,_=read_canonical(bundle_path)
        if bundle.get("identity_class")!="TECHNICAL_MIRROR_ONLY_NEVER_FORMAL_AUTHORITY":raise Repair08Error("TECHNICAL_BUNDLE_CLASS")
        ...
        for entry in bundle["authority_entries"]:
            ...
            if fail_after==count:raise Repair08Error("INJECT_AUTHORITY_READ_FAILURE")
    ...
    if sha_file(allow_path)!=release["authority_allowlist_sha256"]:raise Repair08Error("ALLOWLIST_HASH")
    ...
    for entry in allow["entries"]:
        ...
        if sha_bytes(data)!=entry["sha256"]:raise Repair08Error("AUTHORITY_HASH")
...
def build_staging(stage:Path,objects:Mapping[str,Any],entries:Sequence[Mapping[str,Any]],context:Mapping[str,Any])->dict[str,Any]:
    if stage.exists():raise Repair08Error("STAGE_EXISTS")
...
def terminal_value(context:Mapping[str,Any],success:bool,counts:Mapping[str,int],reason:str,pointer_sha:str|None,phase:str="PHASE_B")->dict[str,Any]:
    return {"schema_version":"gen_enc_fast_b1_repair_08_package_terminal_v1","record_kind":"VERIFIED_SUCCESS" if success else "FAIL_CLOSED","terminal_phase":phase,"repair_id":REPAIR,"task_id":TASK_ID,"batch_id":BATCH_ID,"run_id":context["run_id"],"mode":MODE,"release_sha256":context["release_sha256"],"attestation_sha256":context["attestation_sha256"],"projection_contract_path":PROJECTION_PATH,"projection_contract_sha256":context["projection_contract_sha256"],"dependency_matrix_path":DEPENDENCY_PATH,"dependency_matrix_sha256":context["dependency_matrix_sha256"],"dependency_structure_sha256":context["dependency_structure_sha256"],"status":"VERIFIED_SUCCESS" if success else "TECHNICAL_FAIL_CLOSED","reason":reason,"honest_counts":dict(counts),"commit_pointer_sha256":pointer_sha,"package_terminal_count":1,"claim_ceiling":CLAIM,"scientific_hypothesis_status":"NOT_TESTED","final_test_read":False}
...
def write_terminal(context:Mapping[str,Any],success:bool,counts:Mapping[str,int],reason:str,pointer_sha:str|None=None,phase:str="PHASE_B")->Path:
    ...
    if sp.exists() or fp.exists():raise Repair08Error("SINGLE_TERMINAL")
...
def verify_subject_seal(root:Path,context:Mapping[str,Any],report:Mapping[str,Any],verifier:Mapping[str,Any])->dict[str,Any]:
    if context["verification_subject_seal_path"]!=SUBJECT_SEAL_PATH:raise Repair08Error("SUBJECT_SEAL_CONTROL_PATH")
    ...
    for key,want in expected_binding.items():
        if seal.get(key)!=want:raise Repair08Error("SUBJECT_SEAL_BINDING:"+key)
    paths=verification_subject_paths();entries=[{"path":rel,"sha256":sha_file(root/rel)} for rel in paths];digest=sha_value({"domain":"GEN-ENC-FAST-B1-REPAIR-08-VERIFICATION-SUBJECT-v1","entries":entries})
    if seal["subject_count"]!=27 or seal["subject_entries"]!=entries or seal["subject_digest"]!=digest:raise Repair08Error("SUBJECT_SEAL_SUBJECT_MISMATCH")
    ...
    for name,record in (("REPORT",report),("VERIFIER_TERMINAL",verifier)):
        for key,want in expected.items():
            if record.get(key)!=want:raise Repair08Error("SUBJECT_SEAL_"+name+":"+key)
...
def verified_stage_gate(root:Path,context:Mapping[str,Any])->dict[str,Any]:
    ...
    if actual!=set(logical):raise Repair08Error("VERIFIED_GATE_GRAPH")
    if (root/"SHA256SUMS.txt").read_bytes()!=_hash_lines(root):raise Repair08Error("VERIFIED_GATE_SHA256SUMS")
    members=[]
    for family in FAMILIES:
        for ordinal in range(1,6):
            member,_=read_canonical(root/f"members/{PREFIX[family]}_{ordinal:02d}.json");validate(member,context["schemas"],"member_identity")
            if member["family_id"]!=family or member["slot_ordinal"]!=ordinal:raise Repair08Error("VERIFIED_GATE_MEMBER_ORDER")
    ...
    for family in FAMILIES:
        ...
        if manifest["member_entries"]!=expected or manifest["slot_ordinals"]!=[1,2,3,4,5]:raise Repair08Error("VERIFIED_GATE_MANIFEST")
    audit,_=read_canonical(root/"static_audit.json");validate(audit,context["schemas"],"static_audit")
    if audit["member_paths"]!=[f"members/{m['member_id']}.json" for m in members] or audit["status_counts"]!={s:sum(m["static_status"]==s for m in members) for s in ("ELIGIBLE","COST_INELIGIBLE","TEMPLATE_VALIDITY_REJECTED","FAIL_CLOSED")}:raise Repair08Error("VERIFIED_GATE_AUDIT")
    ...
    if generation["status"]!="AWAITING_INDEPENDENT_VERIFICATION" or generation["member_count"]!=20 or generation["artifact_count"]!=29:raise Repair08Error("VERIFIED_GATE_GENERATION")
    checks=["TYPED_CAD_349_LEAVES","20_MEMBER_DEEP_RECOMPUTE","31_GRAPH_ZERO_UNLISTED"]
    if report["status"]!="PASS" or report["resealed"] is not True or report["verified_member_count"]!=20 or report["verified_manifest_count"]!=4 or report["artifact_count"]!=31 or report["checks"]!=checks:raise Repair08Error("VERIFIED_GATE_REPORT")
    if index["verification_complete"] is not True or index["artifact_cardinality"]!=31 or index["member_paths"]!=[f"members/{m['member_id']}.json" for m in members] or index["partial_manifest_paths"]!=[f"partial_manifests/{PREFIX[f]}.json" for f in FAMILIES]:raise Repair08Error("VERIFIED_GATE_INDEX")
    if verifier["status"]!="VERIFIED_RESEALED_AWAITING_PUBLICATION" or verifier["verified_member_count"]!=20 or verifier["artifact_count"]!=31 or publication!={"schema_version":"gen_enc_fast_b1_repair_08_publication_terminal_v1","record_kind":"PUBLICATION_INTENT_NON_SUCCESS","repair_id":REPAIR,"task_id":TASK_ID,"batch_id":BATCH_ID,"run_id":context["run_id"],"status":"AWAITING_MULTI_TARGET_ATOMIC_PUBLICATION","target_count":31,"published_count":0,"commit_pointer_written":False,"success_terminal_written":False,"claim_ceiling":CLAIM,"final_test_read":False}:raise Repair08Error("VERIFIED_GATE_TERMINALS")
    ...
    for record_name,record in (("INDEX",index),("REPORT",report)):
        for key,want in (("task_id",TASK_ID),("batch_id",BATCH_ID),("run_id",context["run_id"]),("mode",MODE),("projection_contract_path",PROJECTION_PATH),("projection_contract_sha256",context["projection_contract_sha256"]),("dependency_matrix_path",DEPENDENCY_PATH),("dependency_matrix_sha256",context["dependency_matrix_sha256"]),("dependency_structure_sha256",context["dependency_structure_sha256"])):
            if record.get(key)!=want:raise Repair08Error("VERIFIED_GATE_"+record_name+"_BINDING:"+key)
    cumulative=authority_read_count(context)
    if not (audit["authority_read_count_at_generation"]==generation["authority_read_count"]<=report["authority_read_count"]==verifier["authority_read_count"]==cumulative):raise Repair08Error("VERIFIED_GATE_READ_COUNT")
...
def publish(stage:Path,run:Path,journal:Path,pointer_path:Path,context:Mapping[str,Any])->dict[str,Any]:
    ...
    if run.exists():raise Repair08Error("PUBLISH_INPUT")
    run.mkdir(parents=True);entries=[];fault=os.environ.get("GEN_ENC_B1_REPAIR08_FAULT") if context["technical"] else None
    state={"schema_version":"gen_enc_fast_b1_repair_08_publication_journal_v1","repair_id":REPAIR,"task_id":TASK_ID,"batch_id":BATCH_ID,"run_id":context["run_id"],"release_sha256":context["release_sha256"],"run_root_absolute":str(run.resolve()),"pointer_absolute":str(pointer_path.resolve()),"state":"PUBLISHING","entries":entries,"final_test_read":False}
    for index,rel in enumerate(logical):
        ...
        if fault==f"copy:{index}":raise Repair08Error("INJECT_COPY")
        with source.open("rb") as src,temp.open("xb") as dst:shutil.copyfileobj(src,dst);dst.flush();os.fsync(dst.fileno())
        if fault==f"fsync:{index}":raise Repair08Error("INJECT_FSYNC")
        os.replace(temp,target)
        if fault==f"kill:{index}":os._exit(86)
        if fault==f"replace:{index}":raise Repair08Error("INJECT_REPLACE")
        entry["phase"]="REPLACED";atomic_json(journal,state)
        if fault==f"journal:{index}":raise Repair08Error("INJECT_JOURNAL")
...
def _pointer_value(run:Path,context:Mapping[str,Any])->dict[str,Any]:
    value={"schema_version":"gen_enc_fast_b1_repair_08_commit_pointer_v1","record_kind":"ATOMIC_B1_COMMIT_POINTER","repair_id":REPAIR,"task_id":TASK_ID,"batch_id":BATCH_ID,"run_id":context["run_id"],"mode":MODE,"release_sha256":context["release_sha256"],"projection_contract_path":PROJECTION_PATH,"projection_contract_sha256":context["projection_contract_sha256"],"dependency_matrix_path":DEPENDENCY_PATH,"dependency_matrix_sha256":context["dependency_matrix_sha256"],"dependency_structure_sha256":context["dependency_structure_sha256"],"immutable_run_directory":context["roots"]["run"],"batch_index_sha256":sha_file(run/"batch_index.json"),"sha256sums_sha256":sha_file(run/"SHA256SUMS.txt"),"verification_sha256":sha_file(run/"independent_verification.json"),"artifact_count":31,"atomic":True,"final_endpoint":False,"claim_ceiling":CLAIM,"final_test_read":False};validate(value,context["schemas"],"commit_pointer");return value
...
def rollback(journal:Path,run:Path,pointer:Path,context:Mapping[str,Any])->int:
    ...
    if state["state"]!="PUBLISHING" or pointer.exists() or (context["roots_resolved"]["success"]/"VERIFIED_SUCCESS.json").exists():raise Repair08Error("ROLLBACK_STATE_FORBIDDEN")
    ...
    for entry in reversed(state["entries"]):
        ...
        if i>=31 or entry["logical_path"]!=logical[i] or target.resolve()!=(run/logical[i]).resolve() or temp!=_temp(target,state["run_id"],i) or entry["ownership_token"]!=_token(state["run_id"],i,target,temp):raise Repair08Error("RECOVERY_OWNERSHIP")
        for path in (temp,target):
            if not path.resolve().is_relative_to(run.resolve()):raise Repair08Error("RECOVERY_CONTAINMENT")
...
def rollback_postcopy(journal:Path,run:Path,pointer_path:Path,context:Mapping[str,Any])->int:
    ...
    state,_=read_canonical(journal);validate(state,context["schemas"],"journal")
    if state["state"] not in ("PUBLISHING","COMMITTED") or (context["roots_resolved"]["success"]/"VERIFIED_SUCCESS.json").exists():raise Repair08Error("POSTCOPY_ROLLBACK_STATE")
    ...
    if pointer_path.exists():
        ...
        if pointer_value!=_pointer_value(run,context):raise Repair08Error("POSTCOPY_POINTER_OWNERSHIP")
        pointer_path.unlink();removed+=1
    for entry in reversed(state["entries"]):
        i=entry["index"];target=Path(entry["target_absolute"]);temp=Path(entry["temp_absolute"])
        if i>=31 or entry["logical_path"]!=logical[i] or target.resolve()!=(run/logical[i]).resolve() or temp!=_temp(target,state["run_id"],i) or entry["ownership_token"]!=_token(state["run_id"],i,target,temp):raise Repair08Error("POSTCOPY_RECOVERY_OWNERSHIP")
        for path in (temp,target):
            if not path.resolve().is_relative_to(run.resolve()):raise Repair08Error("POSTCOPY_RECOVERY_CONTAINMENT")
            if path.is_file():path.unlink();removed+=1
...
def recover_transaction(context:Mapping[str,Any])->dict[str,Any]:
    ...
    if success.exists():
        if failure.exists() or not journal_path.is_file() or not pointer_path.is_file():raise Repair08Error("RECOVER_SUCCESS_INCOMPLETE")
        state,_=read_canonical(journal_path);validate(state,context["schemas"],"journal")
        if state["state"]!="COMMITTED":raise Repair08Error("RECOVER_SUCCESS_JOURNAL")
    ...
    if state["state"]=="COMMITTED":
        ...
        if len(state["entries"])!=31:raise Repair08Error("RECOVER_COMMITTED_CARDINALITY")
        for i,e in enumerate(state["entries"]):
            target=Path(e["target_absolute"]);temp=Path(e["temp_absolute"])
            if e["index"]!=i or e["logical_path"]!=logical[i] or target.resolve()!=(roots["run"]/logical[i]).resolve() or temp!=_temp(target,state["run_id"],i) or e["ownership_token"]!=_token(state["run_id"],i,target,temp) or e["phase"]!="REPLACED" or sha_file(target)!=e["source_sha256"]:raise Repair08Error("RECOVER_COMMITTED_GRAPH")
        pointer_value=_pointer_value(roots["run"],context)
        if pointer_path.exists():
            existing,_=read_canonical(pointer_path)
            if existing!=pointer_value:raise Repair08Error("RECOVER_POINTER_MISMATCH")
    ...
    if state["state"]=="ROLLED_BACK":
        if pointer_path.exists() or any(p.is_file() for p in roots["run"].rglob("*")):raise Repair08Error("RECOVER_ROLLED_BACK_DIRTY")
    ...
    raise Repair08Error("RECOVER_STATE")
...
def package_results(context:Mapping[str,Any])->dict[str,Any]:
    ...
    if len(terminals)!=1:raise Repair08Error("TERMINAL_CARDINALITY")
    ...
    for key,value in {**_binding(context["release"]),"release_sha256":context["release_sha256"],"attestation_sha256":context["attestation_sha256"]}.items():
        if key in terminal and terminal[key]!=value:raise Repair08Error("PACKAGE_BINDING:"+key)
    cumulative=authority_read_count(context)
    if terminal["honest_counts"]["authority_reads"]!=cumulative:raise Repair08Error("PACKAGE_READ_COUNT")
    if terminal["record_kind"]=="VERIFIED_SUCCESS":
        ...
        if terminal["commit_pointer_sha256"]!=sha_file(pointer_path):raise Repair08Error("PACKAGE_POINTER")
        ...
        for key,value in expected.items():
            if pointer.get(key)!=value:raise Repair08Error("PACKAGE_POINTER_BINDING:"+key)
        run=roots["run"]
        verified_stage_gate(run,context)
        if {p.relative_to(run).as_posix() for p in run.rglob("*") if p.is_file()}!=set(exact_artifacts()):raise Repair08Error("PACKAGE_GRAPH")
        ...
        for record_name,record in (("INDEX",index),("REPORT",report)):
            for key,value in {k:v for k,v in expected.items() if k!="release_sha256"}.items():
                if record.get(key)!=value:raise Repair08Error("PACKAGE_"+record_name+"_BINDING:"+key)
        if (run/"SHA256SUMS.txt").read_bytes()!=_hash_lines(run):raise Repair08Error("PACKAGE_SHA256SUMS")
        if pointer["batch_index_sha256"]!=sha_file(run/"batch_index.json") or pointer["verification_sha256"]!=sha_file(run/"independent_verification.json") or pointer["sha256sums_sha256"]!=sha_file(run/"SHA256SUMS.txt"):raise Repair08Error("PACKAGE_RESEAL")
    else:
        ...
        if terminal["honest_counts"]!=observed:raise Repair08Error("PACKAGE_FAILURE_HISTORICAL_COUNTS")
        if (roots["pointer"]/"B1.json").exists() or any(p.is_file() for p in roots["run"].rglob("*")):raise Repair08Error("PACKAGE_FAILURE_COMMITTED_RESIDUE")
...
def _check_cli_paths(ns:argparse.Namespace,repo:Path,context:Mapping[str,Any],raw_argv:Sequence[str])->None:
    if ns.projection_contract.as_posix()!=PROJECTION_PATH or ns.dependency_matrix.as_posix()!=DEPENDENCY_PATH:raise Repair08Error("ARGV_PROJECTION_PATH")
    if ns.projection_contract.resolve()!=_safe(repo,context["projection_contract_path"]) or ns.dependency_matrix.resolve()!=_safe(repo,context["dependency_matrix_path"]):raise Repair08Error("ARGV_PROJECTION_RESOLUTION")
    if not context["technical"]:
        expected={"repo_root":Path("."),"release":Path("outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_08/control/PHASE_A_RELEASE.json"),"guardian_attestation":Path("outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_08/control/PHASE_A_GUARDIAN_ATTESTATION.json"),"schema_root":Path("schemas/gen_enc/b1_repair_08")}
        if any(getattr(ns,key)!=value for key,value in expected.items()):raise Repair08Error("ARGV_FORMAL_EXACT")
        row=next((x for x in context["command_manifest"]["entries"] if x["command"]==ns.command),None)
        expected_argv=[] if row is None else list(row["argv_after_python"][2:])
        if ns.command in PHASE_B_COMMANDS:
            replacements={"<EXTERNAL_GUARDIAN_AUTHORIZATION_SHA256>":ns.guardian_phase_b_expected_sha256,"<EXTERNAL_GUARDIAN_ATTESTATION_SHA256>":ns.guardian_phase_b_attestation_expected_sha256,"<EXTERNAL_CONTROLLER_DISPATCH_SHA256>":ns.phase_b_dispatch_expected_sha256};expected_argv=[replacements.get(x,x) for x in expected_argv]
        if row is None or list(raw_argv)!=expected_argv:raise Repair08Error("ARGV_BYTE_EXACT")
    ...
    try:
        ...
        if ns.command=="preflight":
            for name in ("staging","run","journal","success","failure","pointer"):
                if roots[name].exists():raise Repair08Error("PREFLIGHT_ROOT_EXISTS:"+name)
            ...
            if context["technical"] and os.environ.get("GEN_ENC_B1_REPAIR08_FAULT")=="post-copy-exception":raise Repair08Error("INJECT_POST_COPY_EXCEPTION")
            terminal=write_terminal(context,True,counts,"POINTER_LAST_PUBLISHED",publication["pointer_sha256"])
            if context["technical"] and os.environ.get("GEN_ENC_B1_REPAIR08_FAULT")=="after-success":os._exit(90)
        ...
        if context is not None and phase_b_validated and ns.command!="package-results":
            ...
            if not sp.exists():
                ...
                if ns.command in PHASE_B_COMMANDS and journal.is_file():
                    ...
                    except BaseException as rollback_exc:raise Repair08Error("FAILURE_ROLLBACK:"+str(rollback_exc)) from rollback_exc
                _observe_counts(roots,counts);counts["published"]=0
                try:
