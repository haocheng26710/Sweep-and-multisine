"""Build strict repair-02 schemas from the frozen typed technical mirror shape."""
...
REPO=Path(__file__).resolve().parents[2];OLD=REPO/"schemas/gen_enc/b1_repair_01";OUT=REPO/"schemas/gen_enc/b1_repair_02"
...
def main()->None:
    ...
    member={"$schema":"https://json-schema.org/draft/2020-12/schema",**infer(members)};member["$id"]="gen_enc_fast_b1_repair_02_member_identity_v1";member["properties"]["schema_version"]={"const":"gen_enc_fast_b1_repair_02_member_identity_v1"};member["properties"]["record_kind"]={"const":"BATCH_LOCAL_MEMBER_IDENTITY_TYPED_CAD_DERIVED"};member["properties"]["task_id"]={"const":driver.TASK_ID};member["properties"]["repair_id"]={"const":driver.REPAIR};member["properties"]["batch_id"]={"const":"B1"};member["properties"]["final_test_read"]={"const":False}
    schemas={"member_identity":member,"typed_projection":{"$schema":"https://json-schema.org/draft/2020-12/schema","$id":"gen_enc_fast_b1_repair_02_typed_projection_v1",**infer([projected])}}
    for path in OLD.glob("*.schema.json"):
        if path.stem=="member_identity.schema":continue
        name=path.name.removesuffix(".schema.json");value=json.loads(path.read_text());value=json.loads(json.dumps(value).replace("repair_01","repair_02").replace("PRE-RELEASE-REPAIR-01","PRE-RELEASE-REPAIR-02"))
        if name in ("release","guardian_attestation","side_record","batch_index"):add_binding(value)
        schemas[name]=value
