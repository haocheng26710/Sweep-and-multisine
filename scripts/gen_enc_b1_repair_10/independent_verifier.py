"""Standalone Repair-10 verifier with an independent short-path gate."""
...
from scripts.gen_enc_b1_repair_10 import cad_projection_verifier as cad
import jsonschema
...
CLAIM="BATCH_LOCAL_IDENTITY_COMPLETE_CAD_STATIC_AND_TECHNICAL_VALIDITY_ONLY";PROJECTION_PATH="outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_10/cad_typed_projection_contract.json";DEPENDENCY_PATH="outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_10/cad_leaf_dependency_matrix.json"
...
MASK=(1<<64)-1;GAMMA=0x9E3779B97F4A7C15;RUN_DOMAIN=b"GEN-ENC-FAST-B1-PRE-RELEASE-REPAIR-10-RUN-ID-v1"
...
class Verify10Error(RuntimeError):pass
...
def _bounds(spec:Mapping[str,Any],axes:Sequence[str],groups:Sequence[str])->dict[str,tuple[float,float]]:
    if tuple(spec.get("parameter_order",()))!=tuple(axes) or spec.get("member_count")!=20 or spec.get("dof")!=len(axes):raise Verify10Error("AXES")
    result={}
    if len(spec.get("parameters",[]))!=len(groups):raise Verify10Error("GROUPS")
    for position,name in enumerate(groups):
        item=spec["parameters"][position]
        if set(item)!={"group","axes","bounds"} or item["group"]!=name or len(item["bounds"])!=2:raise Verify10Error("GROUP")
        lo,hi=float(item["bounds"][0]),float(item["bounds"][1])
        if lo>=hi:raise Verify10Error("BOUND")
        for axis in item["axes"]:
            if axis in result:raise Verify10Error("DUP")
            result[axis]=(lo,hi)
    if list(result)!=list(axes):raise Verify10Error("GROUP_ORDER")
    return result
...
def expected_parameters(objects:Mapping[str,Any],family:str,ordinal:int)->dict[str,float]:
    ...
    if family==FAMILIES[2]:
        ...
        if spec.get("uniform_algorithm")!={"interval":"OPEN_0_1_BINARY64","word_to_uniform":"((word>>11)+0.5)/2^53","one_guard":"nextafter(1.0,0.0)"} or spec.get("splitmix64",{}).get("gamma_hex")!="9e3779b97f4a7c15":raise Verify10Error("RANDOM_SPEC")
    ...
    if spec.get("lhs_algorithm")!=["STRATA=20","SAMPLE=(PERMUTATION[row]+0.5)/20","JITTER=false","FISHER_YATES_SPLITMIX64=i19_TO_1"] or spec.get("lhs_master_seed")!=master:raise Verify10Error("PHYSICS_SPEC")
    out={}
...
def expected_evidence(family:str,p:Mapping[str,float],projected:Mapping[str,Any])->tuple[dict[str,Any],str]:
    ...
    for sector,weight in zip(SECTORS,weights):
        ...
        if abs(volume-target)>float(common["tolerance_m3"]):raise Verify10Error("RESIDUAL")
...
def verify_members(members:Sequence[Mapping[str,Any]],objects:Mapping[str,Any],entries:Sequence[Mapping[str,Any]],contract:Mapping[str,Any],binding:Mapping[str,Any])->dict[str,Any]:
    ...
    if projected["projection_contract_sha256"]!=binding["projection_contract_sha256"] or projected["dependency_matrix_sha256"]!=binding["dependency_structure_sha256"]:raise Verify10Error("PROJECTION_BINDING")
    expected_ids=[f"{PREFIX[f]}_{i:02d}" for f in FAMILIES for i in range(1,6)]
    if [m.get("member_id") for m in members]!=expected_ids:raise Verify10Error("MEMBER_SET_ORDER")
    for member in members:
        ...
        if member["parameters"]!=params or member["cad_static_evidence"]!=evidence or member["typed_projection_provenance"]!=provenance or member["static_status"]!=status:raise Verify10Error("DEEP_MEMBER:"+member["member_id"])
...
def pre_read_binding(record:Mapping[str,Any],expected:Mapping[str,Any])->None:
    for name in ("task_id","batch_id","run_id","projection_contract_path","projection_contract_sha256","dependency_matrix_path","dependency_matrix_sha256","dependency_structure_sha256"):
        if record.get(name)!=expected.get(name):raise Verify10Error("PRE_READ_BINDING:"+name)


def _safe(repo:Path,value:str)->Path:
    result=(repo/value).resolve()
    if result==repo or not result.is_relative_to(repo):raise Verify10Error("PATH_CONTAINMENT")
    return result
...
def _validate(value:Any,schemas:Mapping[str,Any],name:str)->None:
    ...
    except jsonschema.ValidationError as exc:raise Verify10Error("SCHEMA_"+name+":"+exc.message) from exc


def _roots(run_id:str,technical:bool)->dict[str,str]:
    base=(".technical_b1_repair_10/" if technical else "outputs/gen_enc/GEN_ENC_FAST_START/b1_repair_10_runs/")+run_id;result={x:f"{base}/{x}" for x in ("staging","run","journal","success","failure","side_records")};result["pointer"]=f".technical_b1_repair_10_commit/{run_id}" if technical else f"outputs/gen_enc/GEN_ENC_FAST_START/b1_repair_10_commit/{run_id}";return result
...
def _validate_cli_control_addressability(repo:Path,release_path:Path,attestation_path:Path,dispatch_path:Path|None)->None:
    repo=repo.resolve()
    if len(str(repo))>MAX_WORKSPACE_PREFIX_CHARS:raise Verify10Error("CONTROL_WORKSPACE_PREFIX_BUDGET")
    ...
    for key,path in supplied.items():
        ...
        if len(str(absolute))>MAX_CONTROL_ABSOLUTE_CHARS or len(str(absolute))>=WINDOWS_PATH_SAFETY_LIMIT:raise Verify10Error("CONTROL_MAX_PATH:"+key)
        try:relative=absolute.relative_to(repo).as_posix()
        except ValueError as exc:raise Verify10Error("CONTROL_PATH_CONTAINMENT") from exc
        match=re.fullmatch(rf"{CONTROL_ROOT}/([0-9a-f]{{64}})/{roles[key]}",relative)
        if match is None or len(relative)!=CONTROL_RELATIVE_CHARS:raise Verify10Error("CONTROL_SHORT_PATH:"+key)
        run_ids.add(match.group(1));seen.append(relative.casefold())
    if len(run_ids)!=1 or len(seen)!=len(set(seen)):raise Verify10Error("CONTROL_PATH_ROLE_OR_RUN_COLLISION")
...
def _validate_rollover_paths(release:Mapping[str,Any],release_path:Path,attestation_path:Path,dispatch_path:Path|None,repo:Path,technical:bool)->None:
    ...
    if not isinstance(rid,str) or not re.fullmatch(r"[0-9a-f]{64}",rid) or rid in REVOKED_RUN_IDS:raise Verify10Error("ROLLOVER_RUN_ID")
    ...
    for key,value in paths.items():
        if not isinstance(value,str) or "\\" in value or value.startswith("/") or ".." in value.split("/") or not re.fullmatch(patterns[key],value):raise Verify10Error("ROLLOVER_PATH:"+key)
    if len({v.casefold() for v in paths.values()})!=3:raise Verify10Error("ROLLOVER_CASEFOLD_COLLISION")
    ...
    except ValueError as exc:raise Verify10Error("ROLLOVER_PATH_CONTAINMENT") from exc
    if actual_release!=paths["release"] or actual_att!=paths["attestation"]:raise Verify10Error("ROLLOVER_ACTUAL_PATH_BINDING")
    if actual_dispatch is not None and actual_dispatch!=paths["dispatch"]:raise Verify10Error("ROLLOVER_DISPATCH_CLI_BINDING")
    if release.get("phase_a_dispatch_id") in REVOKED_DISPATCH_IDS:raise Verify10Error("ROLLOVER_DISPATCH_ID")
...
def _validate_signing_authority_bytes(repo:Path,release:Mapping[str,Any],technical:bool)->None:
    ...
    for path,want,error in fixed:
        if sha_file(_safe(repo,path))!=want:raise Verify10Error(error)
    for label,path_key,sha_key in (("FINAL_REVIEW","advisor_b_final_review_path","advisor_b_final_review_sha256"),("LEDGER_SNAPSHOT","advisor_b_ledger_snapshot_path","advisor_b_ledger_snapshot_sha256")):
        ...
        if not isinstance(path_value,str) or not path_value.endswith(".json") or not isinstance(expected_sha,str) or len(expected_sha)!=64 or any(c not in "0123456789abcdef" for c in expected_sha):raise Verify10Error("ADVISOR_DYNAMIC_BINDING:"+label)
        ...
        if not path_value.startswith(required_root):raise Verify10Error("ADVISOR_DYNAMIC_ROOT:"+label)
        value,raw=read_canonical(_safe(repo,path_value))
        if sha_bytes(raw)!=expected_sha:raise Verify10Error("ADVISOR_DYNAMIC_HASH:"+label)
        if technical and (value.get("identity_class")!="TASK_OWNED_SYNTHETIC_IMMUTABLE_NEVER_FORMAL" or value.get("subject")!=label or value.get("task_id")!=TASK_ID or value.get("final_test_read") is not False):raise Verify10Error("ADVISOR_TECHNICAL_SYNTHETIC_CLASS:"+label)
...
def gate(repo:Path,release_path:Path,attestation_path:Path,schema_root:Path,cli:argparse.Namespace|None=None,raw_argv:Sequence[str]=())->dict[str,Any]:
    _validate_cli_control_addressability(repo,release_path,attestation_path,None if cli is None else cli.phase_a_dispatch)
    ...
    if technical:
        if release.get("identity_class")!="TECHNICAL_MIRROR_RELEASE_NEVER_FORMAL" or not (repo/".technical_b1_repair_10_root").is_file():raise Verify10Error("TECH_ROOT")
    else:_validate(release,schemas,"release")
    if release.get("record_kind") not in ("PHASE_A_RELEASE","TECHNICAL_MIRROR_RELEASE") or release.get("repair_id")!=REPAIR or release.get("task_id")!=TASK_ID or release.get("batch_id")!=BATCH_ID or release.get("mode")!=MODE or release.get("commands_exact")!=list(PHASE_A_COMMANDS) or release["run_id"]!=derive_run_id(release):raise Verify10Error("PHASE_A_RELEASE")
    if release.get("phase_a_commands_exact")!=list(PHASE_A_COMMANDS) or release.get("phase_a_permissions")!={"preflight":True,"generate_staging":True,"independent_verify":True,"publish":False,"recover":False,"package_results":False}:raise Verify10Error("PHASE_A_PERMISSIONS")
    ...
    if release.get("guardian_two_phase_contract_path")!=GUARDIAN_TWO_PHASE_CONTRACT_PATH or release.get("guardian_two_phase_contract_sha256")!=GUARDIAN_TWO_PHASE_CONTRACT_SHA256 or release.get("guardian_two_phase_review_path")!=GUARDIAN_TWO_PHASE_REVIEW_PATH or release.get("guardian_two_phase_review_sha256")!=GUARDIAN_TWO_PHASE_REVIEW_SHA256:raise Verify10Error("GUARDIAN_TWO_PHASE_AUTHORITY_BINDING")
    if release.get("authority_allowlist_path")!=AUTHORITY_ALLOWLIST_PATH:raise Verify10Error("AUTHORITY_ALLOWLIST_PATH")
    if sha_file(_safe(repo,GUARDIAN_TWO_PHASE_CONTRACT_PATH))!=GUARDIAN_TWO_PHASE_CONTRACT_SHA256 or sha_file(_safe(repo,GUARDIAN_TWO_PHASE_REVIEW_PATH))!=GUARDIAN_TWO_PHASE_REVIEW_SHA256:raise Verify10Error("GUARDIAN_TWO_PHASE_AUTHORITY_BYTES")
    _validate_signing_authority_bytes(repo,release,technical)
    if datetime.fromisoformat(release["expires_at"]).astimezone(timezone.utc)<=datetime.now(timezone.utc) or release["roots"]!=_roots(release["run_id"],technical):raise Verify10Error("EXPIRY_ROOT")
    ...
    if release["projection_contract_path"]!=PROJECTION_PATH or release["dependency_matrix_path"]!=DEPENDENCY_PATH or sha_file(projection_path)!=release["projection_contract_sha256"] or sha_file(matrix_path)!=release["dependency_matrix_sha256"]:raise Verify10Error("PROJECTION_FILE")
    matrix,_=read_canonical(matrix_path)
    if matrix.get("dependency_structure_sha256")!=release["dependency_structure_sha256"] or matrix.get("projection_contract_sha256")!=release["projection_contract_sha256"]:raise Verify10Error("STRUCTURE")
    for name in ("source","schema","command"):
        if sha_file(_safe(repo,release[f"{name}_manifest_path"]))!=release[f"{name}_manifest_sha256"]:raise Verify10Error("MANIFEST:"+name)
    command_manifest,_=read_canonical(_safe(repo,release["command_manifest_path"]))
    rows=command_manifest.get("entries",[])
    if command_manifest.get("mode")!=MODE or command_manifest.get("commands_exact")!=list(COMMANDS) or len(rows)!=6 or [row.get("command") for row in rows]!=list(COMMANDS):raise Verify10Error("COMMAND_MANIFEST")
    ...
    if att.get("record_kind") not in ("GUARDIAN_ONE_WAY_ATTESTATION","TECHNICAL_MIRROR_ONE_WAY_ATTESTATION") or att.get("release_sha256")!=sha_bytes(raw) or att.get("one_way") is not True:raise Verify10Error("ATTESTATION")
    for key,value in expected.items():
        if att.get(key)!=value:raise Verify10Error("ATTESTATION_BINDING:"+key)
    ...
    if failure.exists():
        ...
        for key,want in {"task_id":TASK_ID,"batch_id":BATCH_ID,"run_id":release["run_id"],"mode":MODE,"release_sha256":sha_bytes(raw),"attestation_sha256":sha_bytes(araw),"projection_contract_sha256":release["projection_contract_sha256"],"dependency_matrix_sha256":release["dependency_matrix_sha256"],"dependency_structure_sha256":release["dependency_structure_sha256"]}.items():
            if failed.get(key)!=want:raise Verify10Error("PHASE_A_FAILURE_BINDING:"+key)
        if failed.get("terminal_phase")=="PHASE_A":raise Verify10Error("PHASE_A_FAILURE_PERMANENT_NEW_RUN_REQUIRED")
    ...
    for state in ("PHASE_A_CONSUMED",):
        ...
        if value.get("state")!=state or value.get("phase")!="A" or value.get("dispatch_id")!=release["phase_a_dispatch_id"] or value.get("release_sha256")!=sha_bytes(raw) or value.get("attestation_sha256")!=sha_bytes(araw):raise Verify10Error(state)
        for key,want in expected.items():
            if value.get(key)!=want:raise Verify10Error(state+"_BINDING:"+key)
...
def authority_read_count(context:Mapping[str,Any])->int:
    ...
    if [p.name for p in paths]!=[f"{i:04d}.json" for i in range(1,len(paths)+1)]:raise Verify10Error("READ_RECEIPT_SEQUENCE")
    for ordinal,path in enumerate(paths,1):
        value,_=read_canonical(path);_validate(value,context["schemas"],"authority_read_receipt")
        if value.get("ordinal")!=ordinal:raise Verify10Error("READ_RECEIPT_ORDINAL")
        for key,want in expected.items():
            if value.get(key)!=want:raise Verify10Error("READ_RECEIPT_BINDING:"+key)
    return len(paths)
...
def record_authority_read(context:Mapping[str,Any],purpose:str,path_label:str,expected_sha256:str)->int:
    ...
    value={"schema_version":"gen_enc_fast_b1_repair_10_authority_read_receipt_v1","record_kind":"CUMULATIVE_AUTHORITY_READ_RECEIPT","repair_id":REPAIR,**_receipt_binding(context),"ordinal":ordinal,"phase":"INDEPENDENT_VERIFICATION","purpose":purpose,"path_label":path_label,"expected_sha256":expected_sha256,"final_test_read":False};_validate(value,context["schemas"],"authority_read_receipt")
...
def load_objects(repo:Path,context:Mapping[str,Any])->tuple[dict[str,Any],list[dict[str,Any]],int]:
    ...
    if context["technical"]:
        ...
        if sha_file(bundle_path)!=release["technical_bundle_sha256"]:raise Verify10Error("BUNDLE_HASH")
        bundle,_=read_canonical(bundle_path)
        if bundle.get("identity_class")!="TECHNICAL_MIRROR_ONLY_NEVER_FORMAL_AUTHORITY":raise Verify10Error("BUNDLE_CLASS")
        ...
        for entry in bundle["authority_entries"]:
            ...
            if fail_after==count:raise Verify10Error("INJECT_AUTHORITY_READ_FAILURE")
        ...
        if sha_file(allow_path)!=release["authority_allowlist_sha256"]:raise Verify10Error("ALLOWLIST")
        ...
        for entry in allow["entries"]:
            ...
            if sha_bytes(data)!=entry["sha256"]:raise Verify10Error("AUTHORITY_HASH")
...
def verify_stage(stage:Path,objects:Mapping[str,Any],entries:Sequence[Mapping[str,Any]],context:Mapping[str,Any])->dict[str,Any]:
    ...
    if {p.relative_to(stage).as_posix() for p in stage.rglob("*") if p.is_file()}!=pending:raise Verify10Error("PENDING_GRAPH")
    ...
    for family in FAMILIES:
        ...
        if manifest["member_entries"]!=expected or manifest["slot_ordinals"]!=[1,2,3,4,5]:raise Verify10Error("MANIFEST_GRAPH")
    ...
    if (stage/"SHA256SUMS.txt").read_text(encoding="ascii")!=expected_sums:raise Verify10Error("PENDING_SHA256SUMS")
    for key,want in (("mode",MODE),("projection_contract_path",PROJECTION_PATH),("projection_contract_sha256",context["projection_contract_sha256"]),("dependency_matrix_path",DEPENDENCY_PATH),("dependency_matrix_sha256",context["dependency_matrix_sha256"]),("dependency_structure_sha256",context["dependency_structure_sha256"]),("member_paths",[f"members/{PREFIX[f]}_{i:02d}.json" for f in FAMILIES for i in range(1,6)])):
        if index.get(key)!=want:raise Verify10Error("INDEX:"+key)
    if audit.get("member_count")!=20 or len(audit.get("member_paths",[]))!=20:raise Verify10Error("AUDIT")
    ...
    verifier_terminal={"schema_version":"gen_enc_fast_b1_repair_10_verifier_terminal_v1","record_kind":"VERIFIED_STAGING_NON_PUBLICATION","repair_id":REPAIR,"task_id":TASK_ID,"batch_id":BATCH_ID,"run_id":context["run_id"],"status":"VERIFIED_RESEALED_AWAITING_PUBLICATION","authority_read_count":context["authority_read_count"],"verified_member_count":20,"artifact_count":31,"publication_started":False,"success_terminal_written":False,**subject_binding,"claim_ceiling":CLAIM,"final_test_read":False};_validate(verifier_terminal,context["schemas"],"verifier_terminal");atomic_json(stage/"verifier_terminal.json",verifier_terminal)
    publication={"schema_version":"gen_enc_fast_b1_repair_10_publication_terminal_v1","record_kind":"PUBLICATION_INTENT_NON_SUCCESS","repair_id":REPAIR,"task_id":TASK_ID,"batch_id":BATCH_ID,"run_id":context["run_id"],"status":"AWAITING_MULTI_TARGET_ATOMIC_PUBLICATION","target_count":31,"published_count":0,"commit_pointer_written":False,"success_terminal_written":False,"claim_ceiling":CLAIM,"final_test_read":False};atomic_json(stage/"publication_terminal.json",publication)
    ...
    if len([p for p in stage.rglob("*") if p.is_file()])!=31:raise Verify10Error("VERIFIED_CARDINALITY")
...
def write_phase_a_handoff(stage:Path,context:Mapping[str,Any])->dict[str,Any]:
    ...
    for name in ("run","pointer","success","failure","journal"):
        if roots[name].exists():raise Verify10Error("PHASE_A_ZERO_PUBLICATION:"+name)
    ...
    try:
        with path.open("xb") as stream:stream.write(canonical(value));stream.flush();os.fsync(stream.fileno())
    except FileExistsError as exc:raise Verify10Error("PHASE_A_HANDOFF_ALREADY_EXISTS") from exc
    return value
...
def write_failure(context:Mapping[str,Any],reason:str)->Path:
    ...
    if success.exists():raise Verify10Error("SUCCESS_FORBIDS_FAILURE")
...
def _check_cli_paths(ns:argparse.Namespace,repo:Path,context:Mapping[str,Any],raw_argv:Sequence[str])->None:
    if ns.projection_contract.as_posix()!=PROJECTION_PATH or ns.dependency_matrix.as_posix()!=DEPENDENCY_PATH:raise Verify10Error("ARGV_PROJECTION_PATH")
    if ns.projection_contract.resolve()!=_safe(repo,context["projection_contract_path"]) or ns.dependency_matrix.resolve()!=_safe(repo,context["dependency_matrix_path"]):raise Verify10Error("ARGV_PROJECTION_RESOLUTION")
    ...
    if any(getattr(ns,key)!=value for key,value in expected.items()) or (not context["technical"] and ns.schema_root!=Path("schemas/gen_enc/b1_repair_10")):raise Verify10Error("ARGV_CONTROL_PATH_EXACT")
    ...
    if row is None or list(raw_argv)!=expected_argv:raise Verify10Error("ARGV_BYTE_EXACT")
