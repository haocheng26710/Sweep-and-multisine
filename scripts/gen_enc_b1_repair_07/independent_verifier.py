"""Standalone Repair-07 verifier; imports neither driver nor any old implementation."""
...
from scripts.gen_enc_b1_repair_07 import cad_projection_verifier as cad
import jsonschema
...
CLAIM="BATCH_LOCAL_IDENTITY_COMPLETE_CAD_STATIC_AND_TECHNICAL_VALIDITY_ONLY";PROJECTION_PATH="outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_07/cad_typed_projection_contract.json";DEPENDENCY_PATH="outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_07/cad_leaf_dependency_matrix.json"
...
MASK=(1<<64)-1;GAMMA=0x9E3779B97F4A7C15;RUN_DOMAIN=b"GEN-ENC-FAST-B1-PRE-RELEASE-REPAIR-07-RUN-ID-v1"
...
class Verify07Error(RuntimeError):pass
...
def _bounds(spec:Mapping[str,Any],axes:Sequence[str],groups:Sequence[str])->dict[str,tuple[float,float]]:
    if tuple(spec.get("parameter_order",()))!=tuple(axes) or spec.get("member_count")!=20 or spec.get("dof")!=len(axes):raise Verify07Error("AXES")
    result={}
    if len(spec.get("parameters",[]))!=len(groups):raise Verify07Error("GROUPS")
    for position,name in enumerate(groups):
        item=spec["parameters"][position]
        if set(item)!={"group","axes","bounds"} or item["group"]!=name or len(item["bounds"])!=2:raise Verify07Error("GROUP")
        lo,hi=float(item["bounds"][0]),float(item["bounds"][1])
        if lo>=hi:raise Verify07Error("BOUND")
        for axis in item["axes"]:
            if axis in result:raise Verify07Error("DUP")
            result[axis]=(lo,hi)
    if list(result)!=list(axes):raise Verify07Error("GROUP_ORDER")
    return result
...
def expected_parameters(objects:Mapping[str,Any],family:str,ordinal:int)->dict[str,float]:
    ...
    if family==FAMILIES[2]:
        ...
        if spec.get("uniform_algorithm")!={"interval":"OPEN_0_1_BINARY64","word_to_uniform":"((word>>11)+0.5)/2^53","one_guard":"nextafter(1.0,0.0)"} or spec.get("splitmix64",{}).get("gamma_hex")!="9e3779b97f4a7c15":raise Verify07Error("RANDOM_SPEC")
    ...
    if spec.get("lhs_algorithm")!=["STRATA=20","SAMPLE=(PERMUTATION[row]+0.5)/20","JITTER=false","FISHER_YATES_SPLITMIX64=i19_TO_1"] or spec.get("lhs_master_seed")!=master:raise Verify07Error("PHYSICS_SPEC")
    out={}
...
def expected_evidence(family:str,p:Mapping[str,float],projected:Mapping[str,Any])->tuple[dict[str,Any],str]:
    ...
    for sector,weight in zip(SECTORS,weights):
        ...
        if abs(volume-target)>float(common["tolerance_m3"]):raise Verify07Error("RESIDUAL")
...
def verify_members(members:Sequence[Mapping[str,Any]],objects:Mapping[str,Any],entries:Sequence[Mapping[str,Any]],contract:Mapping[str,Any],binding:Mapping[str,Any])->dict[str,Any]:
    ...
    if projected["projection_contract_sha256"]!=binding["projection_contract_sha256"] or projected["dependency_matrix_sha256"]!=binding["dependency_structure_sha256"]:raise Verify07Error("PROJECTION_BINDING")
    expected_ids=[f"{PREFIX[f]}_{i:02d}" for f in FAMILIES for i in range(1,6)]
    if [m.get("member_id") for m in members]!=expected_ids:raise Verify07Error("MEMBER_SET_ORDER")
    for member in members:
        ...
        if member["parameters"]!=params or member["cad_static_evidence"]!=evidence or member["typed_projection_provenance"]!=provenance or member["static_status"]!=status:raise Verify07Error("DEEP_MEMBER:"+member["member_id"])
...
def pre_read_binding(record:Mapping[str,Any],expected:Mapping[str,Any])->None:
    for name in ("task_id","batch_id","run_id","projection_contract_path","projection_contract_sha256","dependency_matrix_path","dependency_matrix_sha256","dependency_structure_sha256"):
        if record.get(name)!=expected.get(name):raise Verify07Error("PRE_READ_BINDING:"+name)


def _safe(repo:Path,value:str)->Path:
    result=(repo/value).resolve()
    if result==repo or not result.is_relative_to(repo):raise Verify07Error("PATH_CONTAINMENT")
    return result
...
def _validate(value:Any,schemas:Mapping[str,Any],name:str)->None:
    ...
    except jsonschema.ValidationError as exc:raise Verify07Error("SCHEMA_"+name+":"+exc.message) from exc


def _roots(run_id:str,technical:bool)->dict[str,str]:
    base=(".technical_b1_repair_07/" if technical else "outputs/gen_enc/GEN_ENC_FAST_START/b1_repair_07_runs/")+run_id;result={x:f"{base}/{x}" for x in ("staging","run","journal","success","failure","side_records")};result["pointer"]=f".technical_b1_repair_07_commit/{run_id}" if technical else f"outputs/gen_enc/GEN_ENC_FAST_START/b1_repair_07_commit/{run_id}";return result
...
def gate(repo:Path,release_path:Path,attestation_path:Path,schema_root:Path,cli:argparse.Namespace|None=None,raw_argv:Sequence[str]=())->dict[str,Any]:
    ...
    if technical:
        if release.get("identity_class")!="TECHNICAL_MIRROR_RELEASE_NEVER_FORMAL" or not (repo/".technical_b1_repair_07_root").is_file():raise Verify07Error("TECH_ROOT")
    else:_validate(release,schemas,"release")
    if release.get("record_kind") not in ("PHASE_A_RELEASE","TECHNICAL_MIRROR_RELEASE") or release.get("repair_id")!=REPAIR or release.get("task_id")!=TASK_ID or release.get("batch_id")!=BATCH_ID or release.get("mode")!=MODE or release.get("commands_exact")!=list(PHASE_A_COMMANDS) or release["run_id"]!=derive_run_id(release):raise Verify07Error("PHASE_A_RELEASE")
    if release.get("phase_a_commands_exact")!=list(PHASE_A_COMMANDS) or release.get("phase_a_permissions")!={"preflight":True,"generate_staging":True,"independent_verify":True,"publish":False,"recover":False,"package_results":False}:raise Verify07Error("PHASE_A_PERMISSIONS")
    if release.get("guardian_two_phase_contract_path")!=GUARDIAN_TWO_PHASE_CONTRACT_PATH or release.get("guardian_two_phase_contract_sha256")!=GUARDIAN_TWO_PHASE_CONTRACT_SHA256 or release.get("guardian_two_phase_review_path")!=GUARDIAN_TWO_PHASE_REVIEW_PATH or release.get("guardian_two_phase_review_sha256")!=GUARDIAN_TWO_PHASE_REVIEW_SHA256:raise Verify07Error("GUARDIAN_TWO_PHASE_AUTHORITY_BINDING")
    if release.get("authority_allowlist_path")!=AUTHORITY_ALLOWLIST_PATH:raise Verify07Error("AUTHORITY_ALLOWLIST_PATH")
    if sha_file(_safe(repo,GUARDIAN_TWO_PHASE_CONTRACT_PATH))!=GUARDIAN_TWO_PHASE_CONTRACT_SHA256 or sha_file(_safe(repo,GUARDIAN_TWO_PHASE_REVIEW_PATH))!=GUARDIAN_TWO_PHASE_REVIEW_SHA256:raise Verify07Error("GUARDIAN_TWO_PHASE_AUTHORITY_BYTES")
    if datetime.fromisoformat(release["expires_at"]).astimezone(timezone.utc)<=datetime.now(timezone.utc) or release["roots"]!=_roots(release["run_id"],technical):raise Verify07Error("EXPIRY_ROOT")
    ...
    if release["projection_contract_path"]!=PROJECTION_PATH or release["dependency_matrix_path"]!=DEPENDENCY_PATH or sha_file(projection_path)!=release["projection_contract_sha256"] or sha_file(matrix_path)!=release["dependency_matrix_sha256"]:raise Verify07Error("PROJECTION_FILE")
    matrix,_=read_canonical(matrix_path)
    if matrix.get("dependency_structure_sha256")!=release["dependency_structure_sha256"] or matrix.get("projection_contract_sha256")!=release["projection_contract_sha256"]:raise Verify07Error("STRUCTURE")
    for name in ("source","schema","command"):
        if sha_file(_safe(repo,release[f"{name}_manifest_path"]))!=release[f"{name}_manifest_sha256"]:raise Verify07Error("MANIFEST:"+name)
    command_manifest,_=read_canonical(_safe(repo,release["command_manifest_path"]))
    rows=command_manifest.get("entries",[])
    if command_manifest.get("mode")!=MODE or command_manifest.get("commands_exact")!=list(COMMANDS) or len(rows)!=6 or [row.get("command") for row in rows]!=list(COMMANDS):raise Verify07Error("COMMAND_MANIFEST")
    att,araw=read_canonical(attestation_path);expected=_file_binding(release)
    if att.get("record_kind") not in ("GUARDIAN_ONE_WAY_ATTESTATION","TECHNICAL_MIRROR_ONE_WAY_ATTESTATION") or att.get("release_sha256")!=sha_bytes(raw) or att.get("one_way") is not True:raise Verify07Error("ATTESTATION")
    for key,value in expected.items():
        if att.get(key)!=value:raise Verify07Error("ATTESTATION_BINDING:"+key)
    ...
    if failure.exists():
        ...
        for key,want in {"task_id":TASK_ID,"batch_id":BATCH_ID,"run_id":release["run_id"],"mode":MODE,"release_sha256":sha_bytes(raw),"attestation_sha256":sha_bytes(araw),"projection_contract_sha256":release["projection_contract_sha256"],"dependency_matrix_sha256":release["dependency_matrix_sha256"],"dependency_structure_sha256":release["dependency_structure_sha256"]}.items():
            if failed.get(key)!=want:raise Verify07Error("PHASE_A_FAILURE_BINDING:"+key)
        if failed.get("terminal_phase")=="PHASE_A":raise Verify07Error("PHASE_A_FAILURE_PERMANENT_NEW_RUN_REQUIRED")
    issued,_=read_canonical(_safe(repo,release["phase_a_dispatch_path"]));_validate(issued,schemas,"phase_dispatch")
    if issued.get("state")!="PHASE_A_ISSUED" or issued.get("phase")!="A" or issued.get("dispatch_id")!=release["phase_a_dispatch_id"] or issued.get("release_sha256")!=sha_bytes(raw) or issued.get("attestation_sha256")!=sha_bytes(araw):raise Verify07Error("PHASE_A_ISSUED")
    for key,want in expected.items():
        if issued.get(key)!=want:raise Verify07Error("ISSUED_BINDING:"+key)
    if (side/"REVOKED.json").exists():
        revoked,_=read_canonical(side/"REVOKED.json");_validate(revoked,schemas,"side_record")
        if (side/"CONSUMED.json").exists() or revoked.get("state")!="REVOKED" or revoked.get("revoked") is not True:raise Verify07Error("REVOKED_INVALID_STATE")
        for key,want in {**expected,"release_sha256":sha_bytes(raw),"attestation_sha256":sha_bytes(araw)}.items():
            if revoked.get(key)!=want:raise Verify07Error("REVOKED_BINDING:"+key)
        raise Verify07Error("REVOKED_VALID_BOUND")
    for state in ("PHASE_A_CONSUMED",):
        value,_=read_canonical(side/f"{state}.json")
        _validate(value,schemas,"phase_dispatch")
        if value.get("state")!=state or value.get("phase")!="A" or value.get("dispatch_id")!=release["phase_a_dispatch_id"] or value.get("release_sha256")!=sha_bytes(raw) or value.get("attestation_sha256")!=sha_bytes(araw):raise Verify07Error(state)
        for key,want in expected.items():
            if value.get(key)!=want:raise Verify07Error(state+"_BINDING:"+key)
...
def authority_read_count(context:Mapping[str,Any])->int:
    ...
    if [p.name for p in paths]!=[f"{i:04d}.json" for i in range(1,len(paths)+1)]:raise Verify07Error("READ_RECEIPT_SEQUENCE")
    for ordinal,path in enumerate(paths,1):
        value,_=read_canonical(path);_validate(value,context["schemas"],"authority_read_receipt")
        if value.get("ordinal")!=ordinal:raise Verify07Error("READ_RECEIPT_ORDINAL")
        for key,want in expected.items():
            if value.get(key)!=want:raise Verify07Error("READ_RECEIPT_BINDING:"+key)
    return len(paths)
...
def record_authority_read(context:Mapping[str,Any],purpose:str,path_label:str,expected_sha256:str)->int:
    ...
    value={"schema_version":"gen_enc_fast_b1_repair_07_authority_read_receipt_v1","record_kind":"CUMULATIVE_AUTHORITY_READ_RECEIPT","repair_id":REPAIR,**_receipt_binding(context),"ordinal":ordinal,"phase":"INDEPENDENT_VERIFICATION","purpose":purpose,"path_label":path_label,"expected_sha256":expected_sha256,"final_test_read":False};_validate(value,context["schemas"],"authority_read_receipt")
...
def load_objects(repo:Path,context:Mapping[str,Any])->tuple[dict[str,Any],list[dict[str,Any]],int]:
    ...
    if context["technical"]:
        ...
        if sha_file(bundle_path)!=release["technical_bundle_sha256"]:raise Verify07Error("BUNDLE_HASH")
        bundle,_=read_canonical(bundle_path)
        if bundle.get("identity_class")!="TECHNICAL_MIRROR_ONLY_NEVER_FORMAL_AUTHORITY":raise Verify07Error("BUNDLE_CLASS")
        ...
        for entry in bundle["authority_entries"]:
            ...
            if fail_after==count:raise Verify07Error("INJECT_AUTHORITY_READ_FAILURE")
        ...
        if sha_file(allow_path)!=release["authority_allowlist_sha256"]:raise Verify07Error("ALLOWLIST")
        ...
        for entry in allow["entries"]:
            ...
            if sha_bytes(data)!=entry["sha256"]:raise Verify07Error("AUTHORITY_HASH")
...
def verify_stage(stage:Path,objects:Mapping[str,Any],entries:Sequence[Mapping[str,Any]],context:Mapping[str,Any])->dict[str,Any]:
    ...
    if {p.relative_to(stage).as_posix() for p in stage.rglob("*") if p.is_file()}!=pending:raise Verify07Error("PENDING_GRAPH")
    ...
    for family in FAMILIES:
        ...
        if manifest["member_entries"]!=expected or manifest["slot_ordinals"]!=[1,2,3,4,5]:raise Verify07Error("MANIFEST_GRAPH")
    ...
    if (stage/"SHA256SUMS.txt").read_text(encoding="ascii")!=expected_sums:raise Verify07Error("PENDING_SHA256SUMS")
    for key,want in (("mode",MODE),("projection_contract_path",PROJECTION_PATH),("projection_contract_sha256",context["projection_contract_sha256"]),("dependency_matrix_path",DEPENDENCY_PATH),("dependency_matrix_sha256",context["dependency_matrix_sha256"]),("dependency_structure_sha256",context["dependency_structure_sha256"]),("member_paths",[f"members/{PREFIX[f]}_{i:02d}.json" for f in FAMILIES for i in range(1,6)])):
        if index.get(key)!=want:raise Verify07Error("INDEX:"+key)
    if audit.get("member_count")!=20 or len(audit.get("member_paths",[]))!=20:raise Verify07Error("AUDIT")
    ...
    if len([p for p in stage.rglob("*") if p.is_file()])!=31:raise Verify07Error("VERIFIED_CARDINALITY")
...
def write_phase_a_handoff(stage:Path,context:Mapping[str,Any])->dict[str,Any]:
    ...
    for name in ("run","pointer","success","failure","journal"):
        if roots[name].exists():raise Verify07Error("PHASE_A_ZERO_PUBLICATION:"+name)
    ...
    try:
        with path.open("xb") as stream:stream.write(canonical(value));stream.flush();os.fsync(stream.fileno())
    except FileExistsError as exc:raise Verify07Error("PHASE_A_HANDOFF_ALREADY_EXISTS") from exc
    return value
...
def write_failure(context:Mapping[str,Any],reason:str)->Path:
    ...
    if success.exists():raise Verify07Error("SUCCESS_FORBIDS_FAILURE")
    ...
    value={"schema_version":"gen_enc_fast_b1_repair_07_package_terminal_v1","record_kind":"FAIL_CLOSED","terminal_phase":"PHASE_A","repair_id":REPAIR,"task_id":TASK_ID,"batch_id":BATCH_ID,"run_id":context["run_id"],"mode":MODE,"release_sha256":context["release_sha256"],"attestation_sha256":context["attestation_sha256"],"projection_contract_path":PROJECTION_PATH,"projection_contract_sha256":context["projection_contract_sha256"],"dependency_matrix_path":DEPENDENCY_PATH,"dependency_matrix_sha256":context["dependency_matrix_sha256"],"dependency_structure_sha256":context["dependency_structure_sha256"],"status":"TECHNICAL_FAIL_CLOSED","reason":reason,"honest_counts":{"authority_reads":context["authority_read_count"],"members":members,"artifacts":artifacts,"published":0},"commit_pointer_sha256":None,"package_terminal_count":1,"claim_ceiling":CLAIM,"scientific_hypothesis_status":"NOT_TESTED","final_test_read":False};_validate(value,context["schemas"],"package_terminal");failure.parent.mkdir(parents=True,exist_ok=True)
...
def _check_cli_paths(ns:argparse.Namespace,repo:Path,context:Mapping[str,Any],raw_argv:Sequence[str])->None:
    if ns.projection_contract.as_posix()!=PROJECTION_PATH or ns.dependency_matrix.as_posix()!=DEPENDENCY_PATH:raise Verify07Error("ARGV_PROJECTION_PATH")
    if ns.projection_contract.resolve()!=_safe(repo,context["projection_contract_path"]) or ns.dependency_matrix.resolve()!=_safe(repo,context["dependency_matrix_path"]):raise Verify07Error("ARGV_PROJECTION_RESOLUTION")
    if not context["technical"]:
        ...
        if any(getattr(ns,key)!=value for key,value in expected.items()):raise Verify07Error("ARGV_FORMAL_EXACT")
        row=next((x for x in context["command_manifest"]["entries"] if x["command"]=="verify"),None)
        if row is None or list(raw_argv)!=row["argv_after_python"][2:]:raise Verify07Error("ARGV_BYTE_EXACT")
