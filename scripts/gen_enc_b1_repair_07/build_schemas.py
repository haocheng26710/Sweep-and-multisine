"""Build strict Repair-07 two-phase lifecycle and typed-CAD schemas."""
...
from tests.helpers.gen_enc_b1_repair_07_fixture import contract, entries, objects

REPO=Path(__file__).resolve().parents[2];OLD=REPO/"schemas/gen_enc/b1_repair_02";OUT=REPO/"schemas/gen_enc/b1_repair_07"
...
def main()->None:
    ...
    member={"$schema":"https://json-schema.org/draft/2020-12/schema",**infer(members)};member["$id"]="gen_enc_fast_b1_repair_07_member_identity_v1";member["properties"]["schema_version"]={"const":"gen_enc_fast_b1_repair_07_member_identity_v1"};member["properties"]["record_kind"]={"const":"BATCH_LOCAL_MEMBER_IDENTITY_TYPED_CAD_DERIVED"};member["properties"]["task_id"]={"const":driver.TASK_ID};member["properties"]["repair_id"]={"const":driver.REPAIR};member["properties"]["batch_id"]={"const":"B1"};member["properties"]["final_test_read"]={"const":False}
    schemas={"member_identity":member,"typed_projection":{"$schema":"https://json-schema.org/draft/2020-12/schema","$id":"gen_enc_fast_b1_repair_07_typed_projection_v1",**infer([projected])}}
    for path in OLD.glob("*.schema.json"):
        if path.stem=="member_identity.schema":continue
        name=path.name.removesuffix(".schema.json");value=json.loads(path.read_text());value=json.loads(json.dumps(value).replace("repair_02","repair_07").replace("PRE-RELEASE-REPAIR-02","PRE-RELEASE-REPAIR-07"))
    ...
    schemas["package_terminal"]["properties"]["terminal_phase"]={"enum":["PHASE_A","PHASE_B"]};schemas["package_terminal"]["required"].append("terminal_phase")
    ...
    for key,path in (("source_manifest_path","outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_07/source_manifest.json"),("schema_manifest_path","outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_07/schema_manifest.json"),("command_manifest_path","outputs/gen_enc/GEN_ENC_FAST_START/b1_preexecution_repair_07/command_manifest.json")):
        side["properties"][key]={"const":path};side["required"].append(key)
    receipt=copy.deepcopy(schemas["side_record"]);receipt["$id"]="gen_enc_fast_b1_repair_07_authority_read_receipt_v1"
    for key in ("state","one_use","revoked"):receipt["properties"].pop(key);receipt["required"].remove(key)
    receipt["properties"].update({"schema_version":{"const":"gen_enc_fast_b1_repair_07_authority_read_receipt_v1"},"record_kind":{"const":"CUMULATIVE_AUTHORITY_READ_RECEIPT"},"ordinal":{"type":"integer","minimum":1,"maximum":100},"phase":{"enum":["DRIVER_GENERATION","INDEPENDENT_VERIFICATION"]},"purpose":{"type":"string","minLength":1},"path_label":{"type":"string","minLength":1},"expected_sha256":{"type":"string","pattern":"^[0-9a-f]{64}$"}})
    receipt["required"] += ["record_kind","ordinal","phase","purpose","path_label","expected_sha256"]
    schemas["authority_read_receipt"]=receipt
    seal=copy.deepcopy(receipt);seal["$id"]="gen_enc_fast_b1_repair_07_verification_subject_seal_v1"
    for key in ("ordinal","phase","purpose","path_label","expected_sha256"):seal["properties"].pop(key);seal["required"].remove(key)
    seal["properties"].update({"schema_version":{"const":"gen_enc_fast_b1_repair_07_verification_subject_seal_v1"},"record_kind":{"const":"IMMUTABLE_INDEPENDENT_VERIFICATION_SUBJECT_SEAL"},"subject_count":{"const":27},"subject_entries":{"type":"array","minItems":27,"maxItems":27,"items":{"type":"object","additionalProperties":False,"required":["path","sha256"],"properties":{"path":{"type":"string","minLength":1},"sha256":{"type":"string","pattern":"^[0-9a-f]{64}$"}}}},"subject_digest":{"type":"string","pattern":"^[0-9a-f]{64}$"},"verification_nonce":{"type":"string","pattern":"^[0-9a-f]{64}$"}})
    ...
    schemas["phase_a_handoff"]["properties"].update({"state":{"const":"PHASE_A_VERIFIED_HANDOFF_STOP"},"terminal_state":{"const":"VERIFIED_STAGING_AWAITING_GUARDIAN_POSTERIOR_ANCHOR"},"claim_ceiling":{"const":driver.CLAIM},"verification_subject_count":{"const":27},"artifact_graph_count":{"const":31}})
