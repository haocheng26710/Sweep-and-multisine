...
值得，但仅限于已冻结的 HOM/HET 采集问题。预设判断规则为“主频带至少 14/28 个模块对稳定，且至少一个候选窗为广泛稳定”；本轮得到 28/28 和 11 个广泛稳定窗口，明显满足。这个判断说明几何差异足以支持继续做同质/异质阵列对照，不等于方向编码已成立，也不授权分类器优化、SUP-3 或 final-test。本步已在 SUP-1 结束，未实际执行 SUP-2。
