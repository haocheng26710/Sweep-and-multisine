...
| DEV-C1 — P2-A 双入口共享单测量 QC 核心 | 2026-08-05 | 完成 | [DEV-C1_P2A_SHARED_QC_CORE.md](./DEV-C1_P2A_SHARED_QC_CORE.md) | 本报告与实现同一提交 |

当前科研资格边界：截至 DEV-C1，使用的数据只有 `simulated` 和 `external_reference`。两者均不得用于科研结论；只有通过 provenance schema 和 research hard gate 的合格 `real_experiment` 才能进入 `research_analysis`。P2-A 阈值仍为 provisional，P2-B、P3–P6 和 P9 尚未实现，因此不能声明科研结论。
