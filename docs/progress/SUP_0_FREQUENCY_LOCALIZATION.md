...
状态：完成；SUP-1～SUP-4 的候选频带、统计量与排除规则已冻结；HOM/HET 数据身份已按 V2 更正
...
| 同质与异质阵列有何差异 | V2 已确认 HOM-A=U4SYM AS01 B01/B02、HET-ABDF=U4ENC AS01 B03/B04；FORMAL-4/5 已有总体比较 | 对既有数据做方向/固定窗口机理再分析，不重新采集 | SUP-2R |
| 四个位置各自贡献多少 | 未回答 | 固定异质基线上的单位置替换 | SUP-3 |
| 是否可更新论文结论 | FORMAL-5 为 `supported_with_limits` | SUP-1/SUP-2R 只增加频率定位与机理对应；若需单模块因果归因再考虑最小 SUP-3 | SUP-4 |

V2 增量规格（2026-08-24）优先于本报告最初的采集设想：HOM-A 和 HET-ABDF 不是待采集新实验，必须复用上述 B01–B04 权威 ACTIVE 数据。任何旧版本把它们列为 SUP-2 待采集项的表述均由本更正取代。
